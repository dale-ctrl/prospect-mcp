...
function installFetchMock(handler: (call: MockCall) => { status?: number; json?: unknown }) {
  ...
  globalThis.fetch = (async (url: string | URL | Request, init?: RequestInit) => {
    ...
    try {
      parsedBody = rawBody ? (JSON.parse(rawBody) as Record<string, unknown>) : undefined;
    } catch {
      /* not JSON */
    }
    const call: MockCall = {
...
test("addQuoteLine: does NOT send DecimalDiscountPercentage on POST", async () => {
  ...
  try {
    ...
    assert.equal(
      "DecimalDiscountPercentage" in (post!.body ?? {}),
      false,
      "POST body must not include DecimalDiscountPercentage",
    );
    // But DecimalPrice IS still expected (POST honours it)
    assert.equal((post!.body as Record<string, unknown>).DecimalPrice, 100);
  } finally {
    mock.restore();
  }
});
...
test("addQuoteLine: omits the discount PATCH when discountPercentage is 0 or undefined", async () => {
  for (const pct of [undefined, 0] as const) {
    const mock = installFetchMock((call) => {
      if (call.url.includes("/Info()")) return { json: { ProfileId: "p" } };
      if (call.url.includes("/QuoteLines") && call.method === "POST") {
...
test("addQuoteLine: discount PATCH rounds half-percent values correctly", async () => {
  ...
  const mock = installFetchMock((call) => {
    ...
    if (call.url.includes("/QuoteLines(77)") && call.method === "PATCH") return { status: 204 };
    return { json: { value: [] } };
  });
  try {
    await addQuoteLine({
...
test("addQuoteLine: response displays the user-supplied discountPercentage, not the stale created.DecimalDiscountPercentage", async () => {
  ...
  const mock = installFetchMock((call) => {
    ...
    if (call.url.includes("/QuoteLines(88)") && call.method === "PATCH") return { status: 204 };
    return { json: { value: [] } };
  });
  try {
...
test("updateQuoteLine: uses raw `Price` Int (pounds × 100), not `DecimalPrice`", async () => {
  const mock = installFetchMock((call) => {
    if (call.url.includes("/Info()")) return { json: { ProfileId: "p" } };
    if (call.url.includes("/QuoteLines(42)") && call.method === "PATCH") return { status: 204 };
    return { json: { value: [] } };
  });
  try {
