...
export async function searchAutomationProcesses(args: z.infer<typeof searchAutomationProcessesSchema>): Promise<string> {
  const client = getClient();
  const filters: string[] = ["Obsolete eq false"];

  if (args.name) filters.push(`contains(Name,'${args.name}')`);
  if (args.enabledOnly) filters.push("Enabled eq true");
  ...
  const result = await client.get<Record<string, unknown>>("AutomationProcesses", params);
  if (result.value.length === 0) return "No automation processes found.";
  ...
  const lines = result.value.map((p) => {
    ...
    return [
      ...
      p.PublishComment ? `  Comment: ${(p.PublishComment as string).substring(0, 80)}` : "",
    ].filter(Boolean).join("\n");
  });
...
export async function searchAutomationInstances(args: z.infer<typeof searchAutomationInstancesSchema>): Promise<string> {
  const client = getClient();
  const filters: string[] = [];

  if (args.processId) filters.push(`ProcessId eq ${args.processId}`);
  if (args.stateId) filters.push(`contains(StateId,'${args.stateId}')`);
  if (args.leadId) filters.push(`LeadId eq ${args.leadId}`);
  if (args.quoteId) filters.push(`QuoteId eq ${args.quoteId}`);
  if (args.dateFrom) filters.push(`Started ge ${args.dateFrom}`);
  if (args.dateTo) filters.push(`Started le ${args.dateTo}`);
  ...
  const result = await client.get<Record<string, unknown>>("AutomationInstances", params);
  if (result.value.length === 0) return "No automation instances found.";

  const lines = result.value.map((i) => {
    const started = (i.Started as string)?.substring(0, 16).replace("T", " ") || "N/A";
    const refs: string[] = [];
    if (i.QuoteId) refs.push(`Quote #${i.QuoteId}`);
    if (i.LeadId) refs.push(`Lead #${i.LeadId}`);
    if (i.ProblemId) refs.push(`Problem #${i.ProblemId}`);
    ...
    return [
      ...
      i.FailureReason ? `  Error: ${(i.FailureReason as string).substring(0, 100)}` : "",
    ].filter(Boolean).join("\n");
  });
...
export async function searchAutomationSchedules(args: z.infer<typeof searchAutomationSchedulesSchema>): Promise<string> {
  const client = getClient();
  const filters: string[] = [];
  if (args.activeOnly) filters.push("Obsolete eq 0");
  ...
  const result = await client.get<Record<string, unknown>>("AutomationSchedules", params);
  if (result.value.length === 0) return "No automation schedules found.";

  const lines = result.value.map((s) => {
    const days: string[] = [];
    if (s.RunOnMonday) days.push("Mon");
    if (s.RunOnTuesday) days.push("Tue");
    if (s.RunOnWednesday) days.push("Wed");
    if (s.RunOnThursday) days.push("Thu");
    if (s.RunOnFriday) days.push("Fri");
    if (s.RunOnSaturday) days.push("Sat");
    if (s.RunOnSunday) days.push("Sun");

    return [
      `**Schedule #${s.ScheduleId}** — ${s.DisplayName || s.ExtensionId || "(untitled)"}`,
      `  Extension: ${s.ExtensionId || "N/A"} | Interval: ${s.Interval || "N/A"}min`,
      `  Days: ${days.join(", ") || "None"} | Time: ${(s.StartTime as string)?.substring(11, 16) || "?"} – ${(s.EndTime as string)?.substring(11, 16) || "?"}`,
    ].join("\n");
  });
...
export async function searchWebhooks(args: z.infer<typeof searchWebhooksSchema>): Promise<string> {
  const client = getClient();
  const filters: string[] = ["StatusFlag ne 'D'"];

  if (args.entityId) filters.push(`contains(EntityId,'${args.entityId}')`);
  ...
  const result = await client.get<Record<string, unknown>>("Webhooks", params);
  if (result.value.length === 0) return "No webhooks found.";

  const lines = result.value.map((w) => {
    const action = (w.WebhookAction as Record<string, unknown>)?.Description || w.Action || "N/A";
    const url = (w.WebhookUrl as string)?.substring(0, 60) || "N/A";
    return [
      `**Webhook #${w.Id}** — ${w.EntityId || "N/A"} → ${action}`,
      `  URL: ${url}${(w.WebhookUrl as string)?.length > 60 ? "..." : ""}`,
...
export async function getWebhookMessages(args: z.infer<typeof getWebhookMessagesSchema>): Promise<string> {
  const client = getClient();
  const params = [
  ...
  const result = await client.get<Record<string, unknown>>("WebhookMessages", params);
  if (result.value.length === 0) return "No messages found for this webhook.";

  const lines = result.value.map((m) => {
    const date = (m.Created as string)?.substring(0, 16).replace("T", " ") || "N/A";
    const msg = (m.Message as string)?.substring(0, 120) || "N/A";
...
export async function searchImportRuns(args: z.infer<typeof searchImportRunsSchema>): Promise<string> {
  const client = getClient();
  const filters: string[] = [];
  ...
  const result = await client.get<Record<string, unknown>>("ImportRuns", params);
  if (result.value.length === 0) return "No import runs found.";

  const lines = result.value.map((r) => {
    const date = (r.Created as string)?.substring(0, 16).replace("T", " ") || "N/A";
    const status = (r.Status as string)?.substring(0, 80) || "N/A";
...
export async function getImportRunErrors(args: z.infer<typeof getImportRunErrorsSchema>): Promise<string> {
  const client = getClient();
  const params = [
  ...
  const result = await client.get<Record<string, unknown>>("ImportRunErrors", params);
  if (result.value.length === 0) return `No errors found for import run ${args.runId}.`;

  const lines = result.value.map(
    (e) => `- Row ${e.RowNumber}: ${(e.ErrorMessage as string)?.substring(0, 150) || "Unknown error"}`
  );
