...
export async function createProblem(args: z.infer<typeof createProblemSchema>): Promise<string> {
  const client = getClient();
  ...
  if (args.supplierReference !== undefined) body.SupplierReference = args.supplierReference;
  ...
  const created = await client.post<Record<string, unknown>>("Problems", body);
...
export async function updateProblem(args: z.infer<typeof updateProblemSchema>): Promise<string> {
  const client = getClient();
  const { problemId, ...fields } = args;

  const body: Record<string, unknown> = {};
  if (fields.description !== undefined) body.Description = fields.description;
  if (fields.type1Id !== undefined) body.Type1Id = fields.type1Id;
  if (fields.statusId !== undefined) body.StatusId = fields.statusId;
  if (fields.supplierReference !== undefined) body.SupplierReference = fields.supplierReference;
  ...
  await client.patch<Record<string, unknown>>("Problems", problemId, body);

  return `Problem #${problemId} updated successfully. Fields changed: ${Object.keys(body).join(", ")}`;
}

export async function getProblemLookups(): Promise<string> {
  const client = getClient();

  const [types, statuses] = await Promise.all([
    client.get<{ Type1Id: string; Description: string }>(
      "ProblemTypes1", "$select=Type1Id,Description&$orderby=Description"
    ),
    client.get<{ StatusCode: string; Description: string }>(
      "ProblemStatus", "$select=StatusCode,Description&$orderby=Description"
    ),
  ]);
...
export async function searchProblems(args: z.infer<typeof searchProblemsSchema>): Promise<string> {
  const client = getClient();
  ...
  const result = await client.get<Record<string, unknown>>("Problems", params);
  if (result.value.length === 0) return "No problems/tickets found matching the criteria.";

  const lines = result.value.map((p) => {
    const contact = p.Contact as Record<string, unknown> | null;
    ...
    const status = (p.Status as Record<string, unknown>)?.Description || "N/A";
    const responsible = (p.ResponsibleUser as Record<string, unknown>)?.UserName || "N/A";
    const owner = (p.Owner as Record<string, unknown>)?.UserName || "N/A";
    const date = (p.Created as string)?.substring(0, 10) || "N/A";
...
export async function getProblem(args: z.infer<typeof getProblemSchema>): Promise<string> {
  const client = getClient();
  const expand = [
  ...
  const p = await client.getById<Record<string, unknown>>("Problems", args.problemId, `$expand=${expand}`);
  ...
  return [
    ...
    "",
    `## Dates`,
    `- Created: ${(p.Created as string)?.substring(0, 10) || "N/A"}`,
    `- Status Changed: ${(p.StatusChanged as string)?.substring(0, 10) || "N/A"}`,
    `- Last Spoke: ${(p.LastSpoke as string)?.substring(0, 10) || "N/A"}`,
    "",
    `## Situation Summary`,
    (p.SituationSummary as string) || "(none)",
  ].join("\n");
}
