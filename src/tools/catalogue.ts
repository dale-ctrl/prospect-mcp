...
export async function getProductCategories(args: z.infer<typeof getProductCategoriesSchema>): Promise<string> {
  ...
  const result = await client.get<Record<string, unknown>>("ProductCategories", params);
  if (result.value.length === 0) return "No product categories found.";

  const lines = result.value.map(
    (c) => `- \`${c.CategoryId}\` — ${c.Description || "(no description)"}`
  );

  return `## Product Categories (${result.value.length})\n${lines.join("\n")}`;
}

export async function searchProductsByCategory(args: z.infer<typeof searchProductsByCategorySchema>): Promise<string> {
  const client = getClient();
  const params = [
  ...
  if (result.value.length === 0) return `No products found in category "${args.categoryId}".`;
...
export async function getContactPreferences(args: z.infer<typeof getContactPreferencesSchema>): Promise<string> {
  const client = getClient();
  ...
  if (result.value.length === 0) return `No preferences found for contact ${args.contactId}.`;
  ...
  if (flags.length === 0) return `Contact ${args.contactId} has no preference flags set.`;

  return `## Contact Preferences (Contact ${args.contactId})\n${flags.join("\n")}`;
}

export async function getDivisionSalesHistory(args: z.infer<typeof getDivisionSalesHistorySchema>): Promise<string> {
  const client = getClient();

  // Look up division name for display
  const div = await client.getById<Record<string, unknown>>(
    "Divisions", args.divisionId, "$select=DivisionId,Name"
  );

  const filters = [`DivisionId eq ${args.divisionId}`];
  if (args.dateFrom) filters.push(`OrderDate ge ${args.dateFrom}`);
  if (args.dateTo) filters.push(`OrderDate le ${args.dateTo}`);
  ...
  const result = await client.get<Record<string, unknown>>("DivisionSalesHistory", params);
  if (result.value.length === 0) return `No sales history found for ${div.Name}.`;

  let totalValue = 0;
  const lines = result.value.map((h, i) => {
    const date = (h.OrderDate as string)?.substring(0, 10) || "N/A";
...
export async function createInventory(args: z.infer<typeof createInventorySchema>): Promise<string> {
  const client = getClient();
  ...
  if (args.versionNumber !== undefined) body.VersionNumber = args.versionNumber;
  if (args.instances !== undefined) body.Instances = args.instances;
  if (args.extendedDescription !== undefined) body.ExtendedDescription = args.extendedDescription;
  if (args.documentRef !== undefined) body.DocumentRef = args.documentRef;
  if (args.invoiceNumber !== undefined) body.InvoiceNumber = args.invoiceNumber;
  if (args.manufacturerReference !== undefined) body.ManufacturerReference = args.manufacturerReference;
  if (args.contractReference !== undefined) body.ContractReference = args.contractReference;

  const created = await client.post<Record<string, unknown>>("Inventories", body);
...
export async function updateInventory(args: z.infer<typeof updateInventorySchema>): Promise<string> {
  const client = getClient();
  const { inventoryId, ...fields } = args;

  const body: Record<string, unknown> = {};
  if (fields.description !== undefined) body.Description = fields.description;
  if (fields.serialNumber !== undefined) body.SerialNumber = fields.serialNumber;
  if (fields.location !== undefined) body.Location = fields.location;
  if (fields.versionNumber !== undefined) body.VersionNumber = fields.versionNumber;
  if (fields.instances !== undefined) body.Instances = fields.instances;
  if (fields.extendedDescription !== undefined) body.ExtendedDescription = fields.extendedDescription;
  if (fields.documentRef !== undefined) body.DocumentRef = fields.documentRef;
  if (fields.invoiceNumber !== undefined) body.InvoiceNumber = fields.invoiceNumber;
  if (fields.manufacturerReference !== undefined) body.ManufacturerReference = fields.manufacturerReference;
  if (fields.contractReference !== undefined) body.ContractReference = fields.contractReference;

  if (Object.keys(body).length === 0) {
    return "No fields provided to update.";
  }

  await client.patch<Record<string, unknown>>("Inventories", inventoryId, body);

  return `Inventory #${inventoryId} updated successfully. Fields changed: ${Object.keys(body).join(", ")}`;
}

export async function getInventoryLookups(): Promise<string> {
  const client = getClient();

  const [types, statuses] = await Promise.all([
    client.get<Record<string, unknown>>("InventoryTypes", "$select=Code,Description&$orderby=Description"),
    client.get<Record<string, unknown>>("InventoryStatus", "$select=StatusCode,Description&$orderby=Description"),
  ]);
