...
export async function loadCampaignActivities(campaignId: number): Promise<CampaignActivityRef[]> {
  ...
  const result = await client.get<CampaignActivityRef>(
    "CampaignActivities",
    `$filter=CampaignId eq ${campaignId}&$select=CampaignActivityId,CampaignId,Description&$orderby=CampaignActivityId asc`,
  );
  return result.value;
}
...
export async function resolveCampaignActivity(
  ...
  if (activities.length === 0) {
    throw new Error(
      `Campaign ${campaignId} has no activities — cannot link an enquiry to it. ` +
  ...
  if (campaignActivityId !== undefined) {
    ...
    if (!chosen) {
      const list = activities
        ...
        .join("\n");
      throw new Error(
        `CampaignActivityId ${campaignActivityId} does not belong to campaign ${campaignId}. ` +
...
export const linkEnquiryToCampaignSchema = z.object({
  ...
  campaignActivityId: z.number().int().positive().optional().describe(
    "Specific CampaignActivityId under the campaign. Optional — defaults to the campaign's first activity (lowest id).",
  ),
});
...
export const assignEnquirySchema = z.object({
  ...
  assignedTo: z.string().nullable().describe(
    "User to assign — accepts a user code (e.g. 'CL1') or name ('Calvin Liesching', 'Calvin'). " +
...
export async function linkEnquiryToCampaign(
  args: z.infer<typeof linkEnquiryToCampaignSchema>,
): Promise<string> {
  const client = getClient();

  // Validate enquiry exists. getById throws an actionable 404 if not.
  await client.getById<{ EnquiryId: number }>(
    "Enquiries",
    args.enquiryId,
    "$select=EnquiryId",
  );
...
export async function unlinkEnquiryFromCampaign(
  args: z.infer<typeof unlinkEnquiryFromCampaignSchema>,
): Promise<string> {
  const client = getClient();

  const before = await client.getById<{ EnquiryId: number; CampaignActivityId: number | null }>(
    "Enquiries",
    args.enquiryId,
    "$select=EnquiryId,CampaignActivityId",
  );
...
export async function assignEnquiry(
  args: z.infer<typeof assignEnquirySchema>,
): Promise<string> {
  const client = getClient();
...
export async function resolveCampaignAndOwnerFields(
  args: CampaignAndOwnerInputs,
): Promise<ResolvedCampaignAndOwner> {
  const out: ResolvedCampaignAndOwner = { body: {} };
