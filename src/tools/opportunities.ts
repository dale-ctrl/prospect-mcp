...
export async function searchOpportunities(
  args: z.infer<typeof searchOpportunitiesSchema>
): Promise<string> {
  ...
  if (args.contactName) {
    filters.push(
      `(contains(Contact/Forename,'${args.contactName}') or contains(Contact/Surname,'${args.contactName}'))`
    );
  }
  ...
  const result = await client.get<Lead>("Leads", params.join("&"));

  if (result.value.length === 0) return "No opportunities found matching the search criteria.";
...
export async function getOpportunity(args: z.infer<typeof getOpportunitySchema>): Promise<string> {
  const client = getClient();
  const expand = [
  ...
  const lead = await client.getById<Lead>("Leads", args.leadId, `$expand=${expand}`);
...
export async function createOpportunity(
  args: z.infer<typeof createOpportunitySchema>
): Promise<string> {
  const client = getClient();
  ...
  if (divisionId === undefined || addressId === undefined) {
    const contact = await client.getById<Contact & { AddressId?: number }>(
      "Contacts",
      args.contactId,
      "$select=ContactId,DivisionId,AddressId"
    );
    if (divisionId === undefined) divisionId = contact.DivisionId;
    if (addressId === undefined) addressId = (contact as { AddressId?: number }).AddressId;
    if (addressId === undefined) {
      throw new Error(
        `Could not derive AddressId from Contact ${args.contactId}. Please pass addressId explicitly.`
      );
    }
  }
  ...
  if (args.typeId !== undefined) body.TypeId = args.typeId;
  if (args.pipelineId !== undefined) body.PipelineId = args.pipelineId;
  if (args.sourceId !== undefined) body.SourceId = args.sourceId;
  if (args.sourceOther !== undefined) body.SourceOther = args.sourceOther;
  if (args.marginId !== undefined) body.MarginId = args.marginId;
  if (args.salesPersonId !== undefined) body.SalesPersonId = args.salesPersonId;
  if (args.description !== undefined) body.Description = args.description;
  if (args.situationSummary !== undefined) body.SituationSummary = args.situationSummary;
  ...
  if (args.marginValue !== undefined) body.MarginValue = args.marginValue;
  if (args.estimatedClose !== undefined) body.EstimatedClose = args.estimatedClose;
  if (args.autocalculateValue !== undefined) body.AutocalculateValue = args.autocalculateValue;

  const created = await client.post<Lead>("Leads", body as unknown as Record<string, unknown>);
...
export async function updateOpportunity(
  args: z.infer<typeof updateOpportunitySchema>
): Promise<string> {
  const client = getClient();
  const { leadId, ...fields } = args;

  const body: Record<string, unknown> = {};
  if (fields.sizeId !== undefined) body.SizeId = fields.sizeId;
  if (fields.statusId !== undefined) body.StatusId = fields.statusId;
  if (fields.typeId !== undefined) body.TypeId = fields.typeId;
  if (fields.pipelineId !== undefined) body.PipelineId = fields.pipelineId;
  if (fields.sourceId !== undefined) body.SourceId = fields.sourceId;
  if (fields.sourceOther !== undefined) body.SourceOther = fields.sourceOther;
  if (fields.marginId !== undefined) body.MarginId = fields.marginId;
  if (fields.salesPersonId !== undefined) body.SalesPersonId = fields.salesPersonId;
  if (fields.description !== undefined) body.Description = fields.description;
  if (fields.situationSummary !== undefined) body.SituationSummary = fields.situationSummary;
  ...
  if (fields.marginValue !== undefined) body.MarginValue = fields.marginValue;
  if (fields.estimatedClose !== undefined) body.EstimatedClose = fields.estimatedClose;
  if (fields.autocalculateValue !== undefined) body.AutocalculateValue = fields.autocalculateValue;

  if (Object.keys(body).length === 0) {
    return "No fields provided to update. Specify at least one field to change.";
  }

  await client.patch<Lead>("Leads", leadId, body);

  return `✅ Opportunity #${leadId} updated successfully. Fields changed: ${Object.keys(body).join(", ")}`;
}

export async function getLeadLookups(args: z.infer<typeof getLeadLookupsSchema>): Promise<string> {
  const client = getClient();
  ...
  const fetchAndFormat = async (
    ...
    const res = await client.get<{ Code: string; Description: string | null }>(entitySet, params);
    if (res.value.length === 0) return `## ${label}\n(none)`;
