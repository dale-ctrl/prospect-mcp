...
const OBJECT_TYPES = ["division", "contact", "lead", "enquiry", "quote"] as const;
type ObjectTypeCode = (typeof OBJECT_TYPES)[number];

interface ResolvedFKs {
...
async function resolveParentFKs(objectType: ObjectTypeCode, objectId: number): Promise<ResolvedFKs> {
  const client = getClient();
  switch (objectType) {
    case "division": {
      // Validate it exists.
      await client.getById<{ DivisionId: number }>("Divisions", objectId, "$select=DivisionId");
      return { DivisionId: objectId, ContactId: null, EnquiryId: null };
    }
    case "contact": {
      const c = await client.getById<{ ContactId: number; DivisionId: number }>(
        "Contacts", objectId, "$select=ContactId,DivisionId",
      );
      return { DivisionId: c.DivisionId ?? null, ContactId: c.ContactId, EnquiryId: null };
    }
    case "lead": {
      const l = await client.getById<{ LeadId: number; DivisionId: number }>(
        "Leads", objectId, "$select=LeadId,DivisionId",
      );
      return { DivisionId: l.DivisionId ?? null, ContactId: null, EnquiryId: null };
    }
    case "enquiry": {
      const e = await client.getById<{ EnquiryId: number; DivisionId: number }>(
        "Enquiries", objectId, "$select=EnquiryId,DivisionId",
      );
      return { DivisionId: e.DivisionId ?? null, ContactId: null, EnquiryId: e.EnquiryId };
    }
    case "quote": {
      const q = await client.getById<{ QuoteId: number; DivisionId: number }>(
        "Quotes", objectId, "$select=QuoteId,DivisionId",
      );
...
export const createActivityNoteSchema = z.object({
  objectType: z.enum(OBJECT_TYPES).describe(
    "What the note is attached to. One of: division, contact, lead, enquiry, quote.",
  ),
  objectId: z.number().int().positive().describe(
    "Numeric ID of the parent record (DivisionId, ContactId, LeadId, EnquiryId, or QuoteId).",
  ),
  text: z.string().min(1).max(32767).describe(
    "The note body. Plain text; up to 32,767 characters.",
  ),
  dateTime: z.string().optional().describe(
    "When the note is for (ISO timestamp). Defaults to now. Distinct from the audit Created timestamp.",
  ),
  pinned: z.boolean().optional().default(false).describe(
    "Pin to the top of the activity feed for this record.",
  ),
  tags: z.string().optional().describe("Tag string (free-form / comma-separated)."),
  external: z.boolean().optional().default(false).describe(
    "Mark as external/customer-visible. Defaults to false (internal-only).",
  ),
  visibility: z.number().int().optional().default(0).describe(
    "Visibility code. 0 = standard. Higher values restrict who sees the note.",
  ),
  recallUser: z.string().optional().describe(
    "User to recall (follow up). Name or code. Optional — set if you want a follow-up reminder.",
  ),
  recallDateTime: z.string().optional().describe(
    "When to recall the note (ISO timestamp). Required if recallUser is set.",
  ),
  userCode: z.string().optional().describe(
    "Author of the note (CRM user code). Defaults to the connector's PROSPECT_USER_ID. Override only when filing on someone else's behalf.",
  ),
});
...
export async function createActivityNote(
  args: z.infer<typeof createActivityNoteSchema>,
): Promise<string> {
  const client = getClient();
  ...
  let defaultUserCode = "";
  try { defaultUserCode = (loadCredentials().PROSPECT_USER_ID || "").toUpperCase(); } catch { /* PAT missing — already surfaced elsewhere */ }
  ...
  if (fks.DivisionId !== null) body.DivisionId = fks.DivisionId;
  if (fks.ContactId !== null) body.ContactId = fks.ContactId;
  if (fks.EnquiryId !== null) body.EnquiryId = fks.EnquiryId;
  if (args.tags) body.Tags = args.tags;
  if (recallUserCode) body.RecallUserCode = recallUserCode;
  if (args.recallDateTime) body.RecallDateTime = args.recallDateTime;
  ...
  const refs: string[] = [];
  if (created.DivisionId) refs.push(`DivisionId=${created.DivisionId}`);
  if (created.ContactId) refs.push(`ContactId=${created.ContactId}`);
  if (created.EnquiryId) refs.push(`EnquiryId=${created.EnquiryId}`);
...
export async function searchActivityNotes(
  args: z.infer<typeof searchActivityNotesSchema>,
): Promise<string> {
  const client = getClient();
  ...
  if (args.enquiryId) filters.push(`EnquiryId eq ${args.enquiryId}`);
  if (args.objectType) filters.push(`ObjectType eq '${args.objectType}'`);
  if (args.objectId !== undefined) filters.push(`ObjectId eq '${args.objectId}'`);
  if (args.pinnedOnly) filters.push(`Pinned eq true`);
  if (args.dateFrom) filters.push(`DateTime ge ${args.dateFrom}`);
  if (args.dateTo) filters.push(`DateTime le ${args.dateTo}`);
  if (args.user) {
  ...
  const result = await client.get<Record<string, unknown>>("Notepads", params);
  if (result.value.length === 0) return "No activity notes found.";

  const lines = result.value.map((n) => {
    const dt = (n.DateTime as string | null)?.substring(0, 16).replace("T", " ") || (n.Created as string)?.substring(0, 16).replace("T", " ") || "N/A";
    const target = `${n.ObjectType}:${n.ObjectId}`;
    const pinned = n.Pinned ? " 📌" : "";
    const recall = n.RecallUserCode
      ? ` | recall ${n.RecallUserCode} @ ${(n.RecallDateTime as string | null)?.substring(0, 16).replace("T", " ") || "?"}`
      : "";
    const text = ((n.Text as string) || "").replace(/\s+/g, " ").trim();
