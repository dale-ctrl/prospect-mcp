...
// Knowledge files live in WCG_KNOWLEDGE_PATH (shared OneDrive) if set,
// otherwise alongside the server code (per-machine fallback).
const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);
const KNOWLEDGE_DIR = process.env.WCG_KNOWLEDGE_PATH
...
export async function saveQuotingLesson(args: z.infer<typeof saveQuotingLessonSchema>): Promise<string> {
...
export async function saveProductNote(args: z.infer<typeof saveProductNoteSchema>): Promise<string> {
  const timestamp = new Date().toISOString().substring(0, 10);
...
export async function getQuotingKnowledge(): Promise<string> {
...
export async function searchQuotingLessons(args: z.infer<typeof searchQuotingLessonsSchema>): Promise<string> {
  const lessons = readKnowledgeFile(LESSONS_FILE);
  const productNotes = readKnowledgeFile(PRODUCT_NOTES_FILE);
