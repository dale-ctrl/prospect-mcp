...
export const updateDivisionVersaMaintenanceSchema = z.object({
  ...
  equipmentMaintained: z.string().optional().describe(
    "Quantity and Equipment Maintained — writes DivisionXtra.StandardTextField5 " +
    "(e.g. '9 x Versa Benchmark Tables')."
  ),
  totalMaintenanceValue: z.union([z.number(), z.string()]).optional().describe(
    "Total Maintenance Value — writes DivisionXtra.StandardTextField6 (text field). " +
...
export const mergeDivisionDocumentSchema = z.object({
  ...
  quoteTemplateCode: z.string().describe(
    "DocumentTypeCode of the document/PDF template to render (e.g. '23caad' for Versa Maintenance Contract). " +
    "Field name kept as `quoteTemplateCode` for consistency with send_quote_email."
  ),
  emailTemplateCode: z.string().optional().describe(
    "DocumentTypeCode of the cover-email template. If omitted, defaults to '_EMLQC' (the same default send_quote_email uses)."
  ),
  emailTo: z.union([z.string(), z.array(z.string())]).optional().describe(
    "[SAFETY GATE] Caller-supplied recipient is logged but ignored — the email is unconditionally " +
  ...
  emailSubject: z.string().optional().describe(
    "Email subject. If omitted, rendered from the email template's Subject field with placeholders resolved."
  ),
  contactId: z.number().int().positive().optional().describe(
    "ContactId for the merge context. Division.MergeData fails with 'ContactNotSet' " +
 ...
 */
export function formatMaintenanceValue(value: number | string): string {
 ...
 */
async function upsertDivisionXtra(
  divisionId: number,
  ...
  sp.set("$top", "1");
  const result = await client.get<Record<string, unknown>>("DivisionXtras", sp.toString());
  return result.value[0] ?? { DivisionId: divisionId, ...body };
}

// ─── Handlers ────────────────────────────────────────────────────────────────

export async function updateDivisionVersaMaintenance(
  input: z.input<typeof updateDivisionVersaMaintenanceSchema>,
): Promise<string> {
 ...
 */
async function resolveDivisionMergeContactId(
  divisionId: number,
  callerContactId: number | undefined,
): Promise<number> {
  if (callerContactId !== undefined) return callerContactId;
  const client = getClient();
  const sp = new URLSearchParams();
  ...
  sp.set("$top", "5");
  const result = await client.get<{ ContactId: number; Forename: string | null; Surname: string | null }>(
    "Contacts",
    sp.toString(),
  );
  if (result.value.length === 0) {
    throw new Error(
      `merge_division_document: Division ${divisionId} has no active contacts and no contactId was supplied. ` +
  ...
  if (result.value.length > 1) {
    const list = result.value
      ...
      .join(", ");
    throw new Error(
      `merge_division_document: Division ${divisionId} has multiple active contacts and contactId was not supplied. ` +
 ...
 */
async function withDivisionMergeContact<T>(
  divisionId: number,
  contactId: number,
  op: () => Promise<T>,
): Promise<T> {
  ...
  let originalHiddenContactId: number | null = null;
  try {
    const div = await client.getById<{ HiddenContactId: number | null }>(
      "Divisions",
      divisionId,
      "$select=DivisionId,HiddenContactId",
    );
  ...
  try {
    ...
    try {
      ...
      console.error(
        `merge_division_document: failed to restore HiddenContactId on Division ${divisionId} ` +
          `(was: ${originalHiddenContactId}). ` +
          `Restore error: ${(restoreErr as Error).message}. ` +
...
export async function mergeDivisionDocument(
  input: z.input<typeof mergeDivisionDocumentSchema>,
): Promise<string> {
  ...
  if (callerTo) {
    console.error(
      `merge_division_document: recipient overridden — caller requested ` +
  ...
  const result = await withDivisionMergeContact(args.divisionId, contactId, () =>
    sendEntityEmail({
      entitySet: "Divisions",
