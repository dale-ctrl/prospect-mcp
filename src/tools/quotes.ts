...
async function copyQuoteLineXtras(
  client: ReturnType<typeof getClient>,
  sourceLineId: number,
  destLineId: number,
): Promise<number> {
  const xtraRes = await client.get<Record<string, unknown>>(
    "QuoteLineXtras",
  ...
  try {
    await client.patch<Record<string, unknown>>("QuoteLineXtras", destLineId, body);
  } catch (err) {
...
export async function getQuote(args: z.infer<typeof getQuoteSchema>): Promise<string> {
  ...
  const activeLines: QuoteLine[] = [];
  const deletedLines: QuoteLine[] = [];
  for (const l of allLines) {
    const flag = (l as unknown as Record<string, unknown>).StatusFlag;
    if (flag === "D") deletedLines.push(l);
    else activeLines.push(l);
  }
  const seqKey = (l: QuoteLine): [number, number, number] => {
...
export async function createQuote(args: z.infer<typeof createQuoteSchema>): Promise<string> {
  ...
  try {
    await client.patch<Quote>("Quotes", created.QuoteId, { EndDate: priceExpiry });
  } catch (err) {
...
export async function duplicateQuote(args: z.infer<typeof duplicateQuoteSchema>): Promise<string> {
  ...
  for (const line of lines) {
    ...
    const createdLine = await client.post<QuoteLine>("QuoteLines", lineBody);
    linesCopied++;
    ...
    const newLineId =
      typeof (createdLine as unknown as Record<string, unknown>).LineId === "number"
        ? ((createdLine as unknown as Record<string, unknown>).LineId as number)
        : null;
  ...
  const xtrasLine =
    xtrasCopied > 0 || xtrasFailed > 0
      ? `**Line custom fields (Colour/Supplier/etc.) copied:** ${xtrasCopied} line${xtrasCopied === 1 ? "" : "s"}${
          xtrasFailed > 0 ? ` (${xtrasFailed} failed — see server log)` : ""
        }`
      : "";
