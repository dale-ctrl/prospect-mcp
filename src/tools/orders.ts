...
export async function searchOrders(args: z.infer<typeof searchOrdersSchema>): Promise<string> {
  const client = getClient();
  ...
  if (args.orderStatus) filters.push(`contains(OrderStatus,'${args.orderStatus}')`);
  ...
  const result = await client.get<Record<string, unknown>>("SalesOrderHeaders", params);
  if (result.value.length === 0) return "No orders found matching the criteria.";

  const lines = result.value.map((o) => {
    const company = (o.Division as Record<string, unknown>)?.Name || "N/A";
    const salesperson = (o.Salesperson as Record<string, unknown>)?.UserName || "N/A";
    const quote = o.Quote as Record<string, unknown> | null;
    const quoteRef = quote ? `Quote #${quote.QuoteId} (${quote.Description || "N/A"})` : "";
    const date = (o.OrderDate as string)?.substring(0, 10) || "N/A";
    ...
    return [
      ...
      o.ShippedDate ? `  Shipped: ${(o.ShippedDate as string).substring(0, 10)}` : "",
    ].filter(Boolean).join("\n");
  });
...
export async function getOrder(args: z.infer<typeof getOrderSchema>): Promise<string> {
  const client = getClient();
  const expand = [
  ...
  const o = await client.getById<Record<string, unknown>>(
    "SalesOrderHeaders", `OperatingCompanyCode='A',OrderNumber='${args.orderNumber}'`, `$expand=${expand}`
  );

  const company = (o.Division as Record<string, unknown>)?.Name || "N/A";
  const accountCode = (o.Division as Record<string, unknown>)?.SalesLedgerId || "N/A";
  const salesperson = (o.Salesperson as Record<string, unknown>)?.UserName || "N/A";
  const quote = o.Quote as Record<string, unknown> | null;
  ...
  return [
    ...
    `**Customer Ref:** ${o.CustomerReference || "N/A"}`,
    `**Order Date:** ${(o.OrderDate as string)?.substring(0, 10) || "N/A"}`,
    `**Due Date:** ${(o.DueDate as string)?.substring(0, 10) || "N/A"}`,
    `**Shipped:** ${(o.ShippedDate as string)?.substring(0, 10) || "N/A"}`,
...
export async function reportOrdersByDivision(
  args: z.infer<typeof reportOrdersByDivisionSchema>
): Promise<string> {
  const client = getClient();
  ...
  if (result.value.length === 0) return "No orders found for this division.";
