...
export const updateDivisionAddressSchema = z.object({
  divisionId: z.number().int().positive().optional().describe(
    "DivisionId to update. Either `divisionId` or `addressId` is required. " +
  ...
  addressId: z.number().int().positive().optional().describe(
    "AddressId — pass directly if already resolved (e.g. from a prior lookup). " +
  ...
  postcode: z.string().optional().describe(
    "Postcode. Foreign postcodes accepted as-is (no UK-format validation).",
  ),
  country: z.string().optional().describe(
    "Country (e.g. 'United Kingdom', 'Germany', 'Malta'). Optional.",
  ),
});
 ...
 */
function normaliseValue(v: string | undefined): string | undefined {
...
async function resolveAddressIdFromDivision(divisionId: number): Promise<number> {
  const client = getClient();
  const division = await client.getById<{
    DivisionId: number;
    AddressId: number | null;
    MainAddressId: number | null;
    StatusFlag?: string;
  }>("Divisions", divisionId, "$select=DivisionId,AddressId,MainAddressId,StatusFlag");

  if (division.StatusFlag === "D") {
    throw new Error(
      `Division ${divisionId} is soft-deleted (StatusFlag='D') — restore it before editing its address.`,
    );
  }
  ...
  if (!id) {
    throw new Error(
      `Division ${divisionId} has no linked Address record (both AddressId and MainAddressId are null).`,
    );
  }
  return id;
}
...
export async function updateDivisionAddress(
  input: z.input<typeof updateDivisionAddressSchema>,
): Promise<string> {
  ...
  const client = getClient();
  const addressId =
    args.addressId ?? (await resolveAddressIdFromDivision(args.divisionId as number));

  // Build PATCH body. `undefined` keys → skipped. `""` and whitespace-only →
  // explicit clear (forwarded as ""). Trimmed otherwise.
  const body: Record<string, string> = {};
  const changedCamel: string[] = [];
  for (const [camel, pascal] of Object.entries(FIELD_MAP)) {
    const raw = (args as Record<string, unknown>)[camel] as string | undefined;
  ...
  try {
    await client.patch<Record<string, unknown>>("Addresses", addressId, body);
  } catch (err) {
