...
export async function getPriceBands(args: z.infer<typeof getPriceBandsSchema>): Promise<string> {
  ...
  const result = await client.get<Record<string, unknown>>("PriceBands", params);
  if (result.value.length === 0) return "No price bands found.";

  const lines = result.value.map(
    (b) => `- **${b.Description}** (ID: ${b.PriceBandId}) — ${b.CurrencyCode || "GBP"}`
  );

  return `## Price Bands (${result.value.length})\n${lines.join("\n")}`;
}

export async function getPriceBandProductPrices(args: z.infer<typeof getPriceBandProductPricesSchema>): Promise<string> {
  const client = getClient();
  ...
  const result = await client.get<Record<string, unknown>>("PriceBandProductPrices", params);
  if (result.value.length === 0) return "No product prices found for this price band.";
...
export async function searchPriceList(args: z.infer<typeof searchPriceListSchema>): Promise<string> {
  const client = getClient();
  const filters: string[] = ["Obsolete eq 0"];

  if (args.productItemId) filters.push(`ProductItemId eq '${args.productItemId}'`);
  if (args.code) filters.push(`Code eq '${args.code}'`);
  ...
  const result = await client.get<Record<string, unknown>>("PriceLists", params);
  if (result.value.length === 0) return "No price list entries found.";
...
export async function getProductPricing(args: z.infer<typeof getProductPricingSchema>): Promise<string> {
  const client = getClient();
  const sections: string[] = [];

  // 1. Catalogue price from ProductItems
  try {
    const products = await client.get<Record<string, unknown>>(
      "ProductItems",
      `$filter=ProductItemId eq '${args.productItemId}'&$select=ProductItemId,Description,DecimalSellingPrice,DecimalCostPrice,UnitDescription,CategoryId`
    );
  ...
  // 2. Price band prices
  try {
    const bandPrices = await client.get<Record<string, unknown>>(
      "PriceBandProductPrices",
      `$filter=ProductItemId eq '${args.productItemId}'&$select=PriceBandId,DecimalSellingPrice,DecimalCostPrice&$expand=PriceBand($select=Description)&$top=20`
    );
    if (bandPrices.value.length > 0) {
      const lines = bandPrices.value.map((bp) => {
        const band = (bp.PriceBand as Record<string, unknown>)?.Description || `Band ${bp.PriceBandId}`;
  ...
  try {
    const listPrices = await client.get<Record<string, unknown>>(
      "PriceLists",
      `$filter=ProductItemId eq '${args.productItemId}' and Obsolete eq 0&$select=Code,LineDescription,SellingPrice,SellDecimals&$top=20`
    );
