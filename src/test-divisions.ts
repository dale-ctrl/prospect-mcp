...
function installFetchMock(handler: (call: MockCall) => { status?: number; json?: unknown; text?: string }) {
...
function divRow(id: number, overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    DivisionId: id,
    Name: `Division ${id}`,
...
test("search_divisions: customerType filter targets DivisionXtra/StandardDropdownField2 (NOT Division.CustomerType)", async () => {
  ...
  try {
    ...
    const filter = mock.calls[0].query.get("$filter") ?? "";
    assert.ok(
      filter.includes("DivisionXtra/StandardDropdownField2 eq 'M.A.T.'"),
      `expected DivisionXtra/StandardDropdownField2 clause, got: ${filter}`,
    );
    // The dead Division.CustomerType field must NOT be queried.
    assert.ok(
      !/[^/]CustomerType eq/.test(filter),
      `must not target dead Division.CustomerType field: ${filter}`,
    );
    ...
    assert.ok(
      expand.includes("StandardDropdownField2"),
      `expected DivisionXtra select to include StandardDropdownField2: ${expand}`,
    );
  } finally {
    mock.restore();
  }
...
test("list_divisions: customerType filter targets DivisionXtra/StandardDropdownField2", async () => {
  ...
  try {
    ...
    assert.ok(
      filter.includes("DivisionXtra/StandardDropdownField2 eq 'M.A.T.'"),
      `expected DivisionXtra clause, got: ${filter}`,
    );
  } finally {
    mock.restore();
  }
...
test("list_divisions: customDropdown1..5 each map to DivisionXtra/StandardDropdownField{N}", async () => {
  ...
  try {
    ...
    for (let i = 1; i <= 5; i++) {
      assert.ok(
        filter.includes(`DivisionXtra/StandardDropdownField${i} eq 'v${i}'`),
        `slot ${i} missing or wrong: ${filter}`,
      );
    }
  } finally {
    mock.restore();
...
test("search_divisions: '&' in searchTerm is percent-encoded (not treated as query delimiter)", async () => {
  ...
  try {
    ...
    const filter = mock.calls[0].query.get("$filter") ?? "";
    assert.ok(
      filter.includes("contains(Name,'Bath & Wells Multi Academy')"),
      `expected '&' to round-trip inside filter literal, got: ${filter}`,
    );
...
test("search_divisions: territoryCode is case-insensitive via tolower()", async () => {
  ...
  try {
    ...
    const filter = mock.calls[0].query.get("$filter") ?? "";
    assert.ok(
      filter.includes("tolower(TerritoryCode) eq 'wgarea'"),
      `expected lowercase tolower clause, got: ${filter}`,
    );
  } finally {
    mock.restore();
  }
...
test("search_divisions: postcode filter uses startswith on Address/Postcode", async () => {
  ...
  try {
    ...
    const filter = mock.calls[0].query.get("$filter") ?? "";
    assert.ok(
      filter.includes("startswith(Address/Postcode, 'PE10')"),
      `expected startswith on Address/Postcode, got: ${filter}`,
    );
  } finally {
    mock.restore();
  }
...
test("search_divisions: escapes single quotes in searchTerm and filter values", async () => {
  ...
  try {
    ...
    assert.ok(
      filter.includes("DivisionXtra/StandardDropdownField2 eq 'L''Oreal'"),
      `expected escaped customerType (via dropdown2): ${filter}`,
    );
  } finally {
    mock.restore();
  }
...
test("list_divisions: auto-paginates past the 500-row server cap and reports totalCount", async () => {
  ...
  const mock = installFetchMock((call) => {
    ...
    if (skip === 0) body["@odata.count"] = TOTAL;
    return { json: body };
  });
  try {
    ...
    for (const c of mock.calls) {
      ...
      assert.ok(
        f.includes("DivisionXtra/StandardDropdownField2 eq 'M.A.T.'"),
        `customerType filter missing on page: ${f}`,
      );
    }
  } finally {
    mock.restore();
...
test("list_divisions: combines customerType (→ dropdown2) and territoryCode filters via AND", async () => {
  ...
  try {
    ...
    const parsed = JSON.parse(json) as { totalCount: number; returnedCount: number };
    assert.equal(parsed.totalCount, 3);
    assert.equal(parsed.returnedCount, 3);
    const filter = mock.calls[0].query.get("$filter") ?? "";
    assert.ok(
      filter.includes("DivisionXtra/StandardDropdownField2 eq 'M.A.T.'"),
      `customerType missing: ${filter}`,
    );
    assert.ok(
      filter.includes("tolower(TerritoryCode) eq 'wgarea'"),
      `territoryCode missing or wrong shape: ${filter}`,
    );
...
test("list_divisions: explicit skip returns a single page without auto-pagination", async () => {
  ...
  try {
    ...
    const parsed = JSON.parse(json) as {
      ...
      skip: number;
      pageSize: number;
      records: Array<{ DivisionId: number }>;
      truncated: boolean;
    };
...
test("list_divisions: default fields include customDropdown1..5 flattened from DivisionXtra", async () => {
  ...
  try {
    const json = await listDivisions({ filters: { customerType: "M.A.T." }, pageSize: 500 });
    const parsed = JSON.parse(json) as { records: Array<Record<string, unknown>> };
...
test("list_divisions: ceiling at 5000 sets truncated=true when totalCount exceeds it", async () => {
  const mock = installFetchMock((call) => {
    ...
    const body: Record<string, unknown> = { value };
    if (skip === 0) body["@odata.count"] = 8000;
    return { json: body };
  });
  try {
...
test("get_xtra_fields: does NOT send $select (lets API return whatever fields exist)", async () => {
  ...
  try {
    ...
    assert.equal(
      mock.calls[0].query.get("$select"),
      null,
      `get_xtra_fields must not pin a $select clause that includes nonexistent fields like StandardBooleanField`,
    );
