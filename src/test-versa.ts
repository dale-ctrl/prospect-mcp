...
const { updateDivisionVersaMaintenance, mergeDivisionDocument, formatMaintenanceValue } = await import(
  "./tools/versa-maintenance.js"
);
...
function installFetchMock(handler: (call: MockCall) => { status?: number; json?: unknown }) {
  ...
  globalThis.fetch = (async (url: string | URL | Request, init?: RequestInit) => {
    ...
    const call: MockCall = { url: u, method: init?.method ?? "GET", body: parsedBody };
    calls.push(call);
...
test("update_division_versa_maintenance: PATCHes both StandardTextField5/6 with formatted values", async () => {
  const mock = installFetchMock((call) => {
    if (call.method === "PATCH" && call.url.includes("/DivisionXtras(33579)")) return { status: 204 };
  ...
  try {
    ...
    const parsed = JSON.parse(out) as { ok: boolean; fieldsUpdated: string[]; row: Record<string, unknown> };
    assert.equal(parsed.ok, true);
    const patch = mock.calls.find(
      (c) => c.method === "PATCH" && c.url.includes("/DivisionXtras(33579)"),
    );
...
test("update_division_versa_maintenance: only equipmentMaintained → only StandardTextField5 in body", async () => {
  ...
  try {
    ...
    assert.ok(
      !("StandardTextField6" in body),
      `StandardTextField6 must not appear when totalMaintenanceValue omitted: ${JSON.stringify(body)}`,
    );
  } finally {
    mock.restore();
  }
...
test("update_division_versa_maintenance: with no fields returns ok:false without HTTP calls", async () => {
  ...
  try {
    ...
    const parsed = JSON.parse(out) as { ok: boolean; message?: string };
...
const MERGE_RESPONSE_SUBJECT = (subj: string, body: string) => ({
  json: {
...
const MERGE_RESPONSE_NAME = (name: string) => ({
  json: {
    "@odata.context": "$metadata#ProspectSoft.OData.Extensibility.Common.MergeResponse",
...
function installMergeDivisionMock(opts: {
  divisionId: number;
  hiddenContactIdInitial?: number | null;
  contactsForAutoResolve?: Array<{ ContactId: number; Forename?: string; Surname?: string }>;
  sendMessageReturn?: number; // default 8888
  documentId?: number; // default 9999
}) {
  ...
  return installFetchMock((call) => {
    ...
    if (call.method === "POST" && new RegExp(`/Divisions\\(${divisionId}\\)/MergeData\\(\\)$`).test(call.url)) {
      mergeCallNo += 1;
      if (mergeCallNo === 1) return MERGE_RESPONSE_SUBJECT(`Versa Maintenance Contract — DivisionId ${divisionId}`, "Body content");
      if (mergeCallNo === 2) return MERGE_RESPONSE_SIG;
...
test("merge_division_document: drives /Divisions(id)/MergeData + /Divisions(id)/SendMessage with DivisionId on the Document body", async () => {
  ...
  try {
    ...
    const docBody = docPost.body as Record<string, unknown>;
    ...
    const sendBody = sendCalls[0].body as Record<string, unknown>;
...
test("merge_division_document: SAFETY GATE — caller-supplied emailTo is logged but overridden", async () => {
  ...
  try {
    ...
    const parsed = JSON.parse(out) as { to: string; safetyBanner: string };
    ...
    assert.ok(
      errLogs.some((l) => /recipient overridden/.test(l) && /customer@external\.example/.test(l)),
      `expected override log, got: ${errLogs.join(" | ")}`,
    );
...
test("merge_division_document: rejects when SendMessage returns value:0 (helps catch wrong-host config)", async () => {
  ...
  try {
    await assert.rejects(
      () => mergeDivisionDocument({ divisionId: 33579, quoteTemplateCode: "23caad", contactId: 66240 }),
      /SendMessage returned value:0/,
    );
  } finally {
    mock.restore();
  }
...
test("get_division_details: surfaces Versa Maintenance fields when StandardTextField5/6 populated", async () => {
  ...
  const mock = installFetchMock((call) => {
    ...
    if (call.url.includes("/Contacts")) return { json: { value: [] } };
    return { json: { value: [] } };
  });
  try {
...
test("update_division_versa_maintenance: numeric string '280' arriving via JSON-RPC formats to '280.00' on the wire", async () => {
  ...
  try {
    ...
    await updateDivisionVersaMaintenance({
      divisionId: 33579,
      totalMaintenanceValue: "280" as unknown as number,
    });
    const patch = mock.calls.find((c) => c.method === "PATCH");
    const body = patch!.body as Record<string, unknown>;
    assert.equal(
      body.StandardTextField6,
      "280.00",
      `live regression: '280' (string) must format to '280.00': ${JSON.stringify(body)}`,
    );
  } finally {
    mock.restore();
  }
...
test("merge_division_document: with explicit contactId, PATCHes Division.HiddenContactId before merge and restores after", async () => {
  ...
  try {
    ...
    const divisionPatches = mock.calls.filter(
      (c) => c.method === "PATCH" && /\/Divisions\(33579\)$/.test(c.url),
    );
    assert.equal(
      divisionPatches.length,
      2,
      `expected 2 Division PATCHes (set+restore), got ${divisionPatches.length}: ${JSON.stringify(divisionPatches.map((p) => p.body))}`,
    );

    const setBody = divisionPatches[0].body as Record<string, unknown>;
    assert.equal(setBody.HiddenContactId, 66240, `first PATCH must set HiddenContactId: ${JSON.stringify(setBody)}`);

    const restoreBody = divisionPatches[1].body as Record<string, unknown>;
    assert.equal(
      restoreBody.HiddenContactId,
      null,
      `second PATCH must restore HiddenContactId to original (null): ${JSON.stringify(restoreBody)}`,
    );
...
test("merge_division_document: restores the ORIGINAL HiddenContactId, not null, when one was already set", async () => {
  ...
  try {
    ...
    assert.equal((divisionPatches[1].body as Record<string, unknown>).HiddenContactId, 11111, "must restore the pre-existing HiddenContactId, not null");
  } finally {
    mock.restore();
  }
});
...
test("merge_division_document: restores HiddenContactId even if MergeData throws", async () => {
  ...
  const mock = installFetchMock((call) => {
    ...
    if (call.method === "PATCH" && /\/Divisions\(33579\)$/.test(call.url)) return { status: 204 };
    if (call.url.includes("/DocumentTemplates")) return { json: { value: [{ Subject: "X" }] } };
  ...
  try {
    ...
    assert.equal((divisionPatches[1].body as Record<string, unknown>).HiddenContactId, null, "restore must fire even on failure");
...
test("merge_division_document: omitted contactId auto-resolves when Division has exactly 1 active contact", async () => {
  ...
  try {
    ...
    const parsed = JSON.parse(out) as { contactId: number; contactIdResolvedFrom: string };
    ...
    const setPatch = mock.calls.find(
      (c) => c.method === "PATCH" && /\/Divisions\(33579\)$/.test(c.url),
    );
    assert.equal((setPatch!.body as Record<string, unknown>).HiddenContactId, 99999);
  } finally {
    mock.restore();
  }
});
...
test("merge_division_document: omitted contactId errors clearly when Division has 0 active contacts", async () => {
  ...
  try {
    await assert.rejects(
      () => mergeDivisionDocument({ divisionId: 33579, quoteTemplateCode: "23caad" }),
      /no active contacts.*Pass contactId explicitly/,
    );
...
test("merge_division_document: omitted contactId errors clearly when Division has multiple active contacts", async () => {
  ...
  try {
    await assert.rejects(
      () => mergeDivisionDocument({ divisionId: 33579, quoteTemplateCode: "23caad" }),
      /multiple active contacts.*11111.*22222/s,
    );
  } finally {
    mock.restore();
  }
