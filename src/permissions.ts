...
export interface PermissionsConfig {
  ...
  modules?: Array<{ id: string; label: string; description?: string; actions?: string[] }>;
}
 ...
 */
function validatePermissionsSchema(parsed: unknown): PermissionsConfig {
  ...
  return obj as unknown as PermissionsConfig;
}
...
function writeCacheAtomically(path: string, content: string): void {
...
export async function loadPermissions(): Promise<PermissionsConfig> {
  ...
  try {
    ...
    try {
      ...
      logWarn(`remote OK but cache write failed (${(cacheErr as Error).message}) — continuing without cache update`);
    }
    logInfo(`loaded from remote ${PERMISSIONS_URL}, cached at ${CACHE_PATH}`);
    return parsed;
  } catch (err) {
    logWarn(`remote fetch from ${PERMISSIONS_URL} failed (${(err as Error).message}) — falling back to cache`);
  }
  ...
  if (existsSync(CACHE_PATH)) {
    try {
      ...
      logWarn(`cache at ${CACHE_PATH} unreadable (${(err as Error).message}) — falling back to bundled defaults`);
    }
  } else {
 ...
 */
export function loadPermissionsSync(): PermissionsConfig | null {
  if (existsSync(CACHE_PATH)) {
    try {
