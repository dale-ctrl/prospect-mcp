 ...
 */
function dropdownItemsStub(call: MockCall) {
  if (!call.url.includes("/DropdownItems")) return null;
...
function withDropdowns(
  inner: (call: MockCall) => { status?: number; json?: unknown; text?: string },
...
test("update_division: customerType writes to DivisionXtras (not Divisions.CustomerType)", async () => {
  ...
  try {
    ...
    const divisionPatches = mock.calls.filter(
      (c) => c.method === "PATCH" && /\/Divisions\(/.test(c.url),
    );
    ...
    const xtraPatches = mock.calls.filter(
    ...
    const body = xtraPatches[0].body as Record<string, unknown>;
...
test("update_division: regular fields and customerType produce two PATCHes (Divisions + DivisionXtras)", async () => {
  ...
  try {
    ...
    const divBody = divPatch.body as Record<string, unknown>;
    ...
    const xtraBody = xtraPatch.body as Record<string, unknown>;
...
test("create_division: customerType triggers a follow-up PATCH to DivisionXtras (NOT in Divisions POST body)", async () => {
  ...
  try {
    ...
    assert.ok(divPost);
    const divBody = divPost.body as Record<string, unknown>;
...
test("create_division: explicitly POSTs a Company first (does not rely on auto-create)", async () => {
  ...
  try {
    ...
    const cBody = companyPost.body as Record<string, unknown>;
    ...
    const divPost = mock.calls.find((c) => c.method === "POST" && c.url.endsWith("/Divisions"));
    const dBody = divPost!.body as Record<string, unknown>;
...
test("update_division_xtra: 'Full Delivery Address' label resolves to StandardMemoField3 in PATCH body", async () => {
  ...
  const mock = installFetchMock((call) => {
    ...
    if (call.url.includes("/EntityFields")) return DIVISION_XTRA_ENTITY_FIELDS;
    ...
    if (call.url.includes("/DivisionXtras(5380)") && call.method === "PATCH") return { status: 204 };
  ...
  try {
    ...
    const parsed = JSON.parse(out) as { ok: boolean; divisionId: number; fieldsUpdated: string[] };
...
test("update_division_xtra: rejects an unknown field key with the resolver's helpful list of valid slots", async () => {
  ...
  try {
    await assert.rejects(
      () => updateDivisionXtra({ divisionId: 5380, fields: { Bogus: "x" } }),
      /Unknown Xtra field.*Bogus/i,
    );
  } finally {
    mock.restore();
  }
