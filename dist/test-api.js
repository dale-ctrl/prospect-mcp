...
async function main() {
    ...
    try {
        ...
        const statuses = await client.get("QuoteStatus", "$select=QuoteStatusCode,Description,DeadFlag");
        ...
        const quotes = await client.get("Quotes", "$select=QuoteId,Description,Created&$orderby=Created desc&$top=3&$filter=StatusFlag ne 'D'");
        ...
        const products = await client.get("ProductItems", "$select=ProductItemId&$top=1&$count=true");
