...
const { listDropdownOptions, deleteDivision, __resetDropdownCache } = await import("./tools/dropdowns.js");
...
test("list_dropdown_options(field='customerType') returns options with FK code + label", async () => {
    ...
    try {
        ...
        assert.ok(filter.includes("startswith(Id, 'Entity.DivisionXtra.StandardDropdownField2.')"), `filter must be slot-2 scoped: ${filter}`);
...
test("list_dropdown_options rejects unknown field with helpful error", async () => {
    ...
    try {
        await assert.rejects(() => listDropdownOptions({ field: "bogusField", includeObsolete: false }), /Unknown dropdown field 'bogusField'/);
    }
    finally {
        mock.restore();
    }
...
test("update_division(customerType='M.A.T.') translates label to FK before PATCH", async () => {
    ...
    try {
        ...
        const body = xtraPatch.body;
        assert.equal(body.StandardDropdownField2, MAT_FK, `body must contain the FK, not the raw label: ${JSON.stringify(body)}`);
    }
    finally {
        mock.restore();
    }
...
test("update_division(customerType='Bogus Label') throws with available labels listed", async () => {
    ...
    try {
        await assert.rejects(() => updateDivision({ divisionId: 1231, customerType: "Bogus Label" }), /Could not resolve 'Bogus Label'.*M\.A\.T\./s);
    }
    finally {
        mock.restore();
    }
...
test("delete_division: refuses without confirmed=true", async () => {
    ...
    try {
        await assert.rejects(() => deleteDivision({ divisionId: 34020, confirmed: false }), /destructive.*confirmed=true/i);
...
//# sourceMappingURL=test-dropdowns.js.map