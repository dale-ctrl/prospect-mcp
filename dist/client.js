...
export function loadCredentials() {
    ...
    if (!env.PROSPECT_PAT) {
        ...
        if (fs.existsSync(configPath)) {
            ...
            try {
                ...
                throw new Error(`Could not read Prospect plugin config at ${configPath}: ${err.message}. ` +
            ...
            try {
                ...
                throw new Error(`Prospect plugin config at ${configPath} is not valid JSON: ${err.message}. ` +
    ...
    if (!result.PROSPECT_PAT) {
        throw new Error("PROSPECT_PAT not configured. Either set the PROSPECT_PAT env var, or run the plugin setup:\n" +
...
export class ProspectClient {
    ...
    userId;
    apiUserEmail = null;
    apiUserEmailError = null;
    apiUserEmailPromise = null;
    retryDelayMs = 1000;
    maxRetries = 3;
    constructor() {
     ...
     */
    async getById(entitySet, id, queryParams) {
        const url = queryParams
     ...
     */
    async getBinary(pathAfterBase) {
        await this.ensureProfileId();
        const url = `${this.baseUrl}/${pathAfterBase}`;
        const res = await fetch(url, { method: "GET", headers: this.headers });
        if (!res.ok) {
            throw new Error(`Prospect API error: HTTP ${res.status} ${res.statusText}\nURL: ${url}\nResponse: ${await res.text()}`);
        }
     ...
     */
    async invokeAction(entitySet, id, actionName, body = {}) {
     ...
     */
    async invokeCollectionAction(entitySet, actionName, body = {}) {
    ...
    async fetchWithRetry(url, init, attempt = 0) {
        ...
        if (!contentType.includes("json")) {
            if (!response.ok) {
                throw new Error(`Prospect API error: HTTP ${response.status} ${response.statusText}\n` +
                    `URL: ${url}\n` +
     ...
     */
    async getApiUserEmail() {
        if (this.apiUserEmail)
            return this.apiUserEmail;
        if (this.apiUserEmailError)
            throw this.apiUserEmailError;
        if (this.apiUserEmailPromise)
            return this.apiUserEmailPromise;
        this.apiUserEmailPromise = (async () => {
            const userId = (this.userId || "").trim();
            if (!userId) {
                throw new Error("PROSPECT_USER_ID is not configured — cannot resolve the API user's email " +
            ...
            const res = await this.get("Users", `$filter=UserCode eq '${userId.replace(/'/g, "''")}'&$select=UserCode,EmailAddress&$top=1`);
            const row = res.value[0];
            if (!row) {
                throw new Error(`No CRM user found with UserCode '${userId}' — cannot resolve API user email for safety gate.`);
            }
            const email = (row.EmailAddress || "").trim();
            if (!email) {
                throw new Error(`CRM user '${userId}' has no EmailAddress on file — cannot resolve API user email for safety gate.`);
            }
