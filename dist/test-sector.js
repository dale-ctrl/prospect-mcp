...
const { createDivision, updateDivision, updateDivisionSchema, createDivisionSchema } = await import("./tools/contacts.js");
const { listDropdownOptions, __resetDropdownCache, DROPDOWN_FIELDS } = await import("./tools/dropdowns.js");
...
test("list_dropdown_options(field='sector') queries DivisionLimiteds and returns EDUCATION", async () => {
    const mock = installFetchMock((call) => {
        if (call.url.includes("/DivisionLimiteds"))
            return { json: { value: SECTOR_OPTIONS } };
        return { json: { value: [] } };
    });
    try {
...
test("update_division(sector='EDUCATION'): label translates to LimitedId='a9ef19'", async () => {
    const mock = installFetchMock((call) => {
        if (call.url.includes("/DivisionLimiteds"))
            return { json: { value: SECTOR_OPTIONS } };
        if (call.method === "PATCH" && call.url.includes("/Divisions(2069)"))
            return { status: 204 };
        return { status: 204 };
    });
    try {
        ...
        assert.ok(!("StandardIndustryCode" in body), `sector must not write to StandardIndustryCode: ${JSON.stringify(body)}`);
    }
    finally {
        mock.restore();
    }
...
test("create_division: sector + standardIndustryCode together write LimitedId AND StandardIndustryCode (independent fields)", async () => {
    ...
    try {
        ...
        const body = divPost.body;
        assert.equal(body.LimitedId, "a9ef19", `SECTOR (LimitedId) missing: ${JSON.stringify(body)}`);
        assert.equal(body.StandardIndustryCode, "318b5d", `SCHOOL STATUS (StandardIndustryCode) missing: ${JSON.stringify(body)}`);
    }
    finally {
        mock.restore();
    }
...
test("update_division(territoryCode='...'): keeps writing to Division.TerritoryCode (path unchanged from round 1)", async () => {
    ...
    try {
        ...
        assert.ok(patch);
        const body = patch.body;
        assert.equal(body.TerritoryCode, "WGAREA137e6eff02e14d98942fe6b8baf5af77", `TerritoryCode must round-trip: ${JSON.stringify(body)}`);
    }
    finally {
        mock.restore();
    }
...
test("schema descriptions: territoryCode reads 'AREA LOCATION', standardIndustryCode reads 'SCHOOL STATUS'", () => {
    ...
    for (const shape of [update, create]) {
        ...
        assert.ok(/AREA LOCATION/i.test(territoryDesc), `territoryCode description should mention 'AREA LOCATION', got: ${territoryDesc}`);
        const sicDesc = shape.standardIndustryCode._def.description ?? "";
        assert.ok(/SCHOOL STATUS/i.test(sicDesc), `standardIndustryCode description should mention 'SCHOOL STATUS', got: ${sicDesc}`);
        assert.ok(!/^SECTOR/i.test(sicDesc), `standardIndustryCode description should no longer start with 'SECTOR': ${sicDesc}`);
        const sectorDesc = shape.sector._def.description ?? "";
        assert.ok(/SECTOR/i.test(sectorDesc) && /LimitedId/.test(sectorDesc), `sector description should mention SECTOR + LimitedId: ${sectorDesc}`);
    }
});
...
test("inspect_division_categorisation_panel: hint mentions correct backing fields and Interiors AM caveat", async () => {
    ...
    try {
        ...
        assert.ok(parsed.hint.includes("Division.StandardIndustryCode"), `hint must mention StandardIndustryCode for SCHOOL STATUS: ${parsed.hint}`);
        assert.ok(parsed.hint.includes("Interiors Account Manager"), `hint must explain Interiors AM is not in OData surface: ${parsed.hint}`);
        // No longer says "look on divisionXtra/companyXtra for AREA LOCATION" — that was the wrong hint.
        assert.ok(!/divisionXtra' or 'companyXtra' whose values match the AREA LOCATION/.test(parsed.hint), `old misleading hint must be gone: ${parsed.hint}`);
    }
    finally {
        mock.restore();
    }
...
//# sourceMappingURL=test-sector.js.map