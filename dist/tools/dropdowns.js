...
export const listDropdownOptionsSchema = z.object({
    field: z.string().describe(`Which dropdown to list options for. One of: ${FIELD_ALIASES.join(", ")}. ` +
        "Returns active (non-obsolete) options only."),
    includeObsolete: z.boolean().optional().default(false).describe("Include rows where Obsolete=1 (default false)."),
});
/**
...
export function __resetDropdownCache() {
    optionsCache.clear();
}
// ─── Handlers ────────────────────────────────────────────────────────────────
export async function fetchDropdownOptions(field, includeObsolete = false) {
    ...
    if (!source) {
        throw new Error(`Unknown dropdown field '${field}'. Valid options: ${FIELD_ALIASES.join(", ")}`);
    }
    const filterClauses = [];
    if (source.extraFilter)
        filterClauses.push(source.extraFilter);
    if (!includeObsolete)
        filterClauses.push("Obsolete eq 0");
    const sp = new URLSearchParams();
    if (filterClauses.length > 0)
        sp.set("$filter", filterClauses.join(" and "));
    sp.set("$select", `${source.keyField},Description,Obsolete`);
    sp.set("$orderby", "Description");
    sp.set("$top", "500");
    const client = getClient();
    const result = await client.get(source.entitySet, sp.toString());
    const options = result.value.map((row) => ({
...
export async function listDropdownOptions(args) {
 ...
 */
export async function resolveDropdownValue(field, rawValue) {
    ...
    throw new Error(`Could not resolve '${value}' for field '${field}'. ` +
...
export const deleteDivisionSchema = z.object({
    ...
    confirmed: z.boolean().describe("Must be `true` to actually delete. This is a guardrail — without it the call returns an error."),
});
export async function deleteDivision(args) {
    if (args.confirmed !== true) {
        throw new Error(`delete_division is destructive. Pass confirmed=true to actually delete DivisionId ${args.divisionId}. ` +
