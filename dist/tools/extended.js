...
function formatXtraData(data) {
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        if (dateVal != null)
            lines.push(`**Date ${i}:** ${dateVal?.substring?.(0, 10) || dateVal}`);
 ...
 */
export async function getContactDetails(args) {
    ...
    const contact = await client.getById("Contacts", args.contactId, `$expand=${expand}`);
    const div = contact.Division;
    const addr = div?.Address;
    const xtra = contact.ContactXtra;
    const address = addr
 ...
 */
export async function getDivisionDetails(args) {
    const client = getClient();
    const expand = [
    ...
    const div = await client.getById("Divisions", args.divisionId, `$expand=${expand}`);
    const addr = div.Address;
    const am = div.AccountManagerUser;
    const xtra = div.DivisionXtra;
    const address = addr
        ? [addr.AddressLine1, addr.AddressLine2, addr.AddressLine3, addr.AddressLine4, addr.AddressLine5, addr.Postcode, addr.Country]
            .filter(Boolean).join(", ")
        : "N/A";
    // Also fetch top contacts for this division
    const contacts = await client.get("Contacts", `$filter=DivisionId eq ${args.divisionId} and StatusFlag ne 'D'&$select=ContactId,Forename,Surname,Email,JobTitle,PhoneNumber&$top=10&$orderby=Surname`);
    ...
    const equipmentMaintained = (xtra?.StandardTextField5 ?? null);
    const totalMaintenanceValue = (xtra?.StandardTextField6 ?? null);
    const output = [
        `# Division Details`,
        `**Name:** ${div.Name || "N/A"}`,
    ...
    if (equipmentMaintained || totalMaintenanceValue) {
        ...
        if (equipmentMaintained)
            output.push(`**Equipment Maintained:** ${equipmentMaintained}`);
        if (totalMaintenanceValue)
            output.push(`**Total Maintenance Value:** ${totalMaintenanceValue}`);
    }
    if (xtra) {
        output.push("", "## Custom Fields (Xtra)");
        output.push(formatXtraData(xtra));
...
export async function searchLeads(args) {
    ...
    const result = await client.get("Leads", params);
    if (result.value.length === 0) {
 ...
 */
export async function getLeadDetails(args) {
    const client = getClient();
    const expand = [
    ...
    if (quotes && quotes.length > 0) {
        const quoteLines = quotes.map((q) => {
            ...
            return `  - Quote #${q.QuoteId}: ${q.Description || "(no desc)"} — ${qStatus?.Description || "?"} — Net £${q.DecimalHomeNetValue?.toFixed(2) ?? "0.00"}`;
        });
