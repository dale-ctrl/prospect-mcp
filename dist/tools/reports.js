...
export async function resolveUserCodes(inputs) {
    ...
    for (const input of inputs) {
        ...
        const byCode = _userCache.find((u) => u.UserCode.toUpperCase() === trimmed.toUpperCase());
        if (byCode) {
    ...
    if (unmatched.length > 0) {
        const available = _userCache
            ...
            .join("\n");
        throw new Error(`Could not match these users: ${unmatched.join(", ")}\n\nAvailable users:\n${available}`);
    }
 ...
 */
async function fetchAll(entitySet, queryParams, pageSize = 500, maxPages = 50) {
    const client = getClient();
    const all = [];
    for (let page = 0; page < maxPages; page++) {
        const skip = page * pageSize;
        const params = `${queryParams}&$top=${pageSize}&$skip=${skip}`;
        const result = await client.get(entitySet, params);
...
export const reportAccountsWithoutTasksSchema = z.object({
    territoryDescription: z.string().optional().describe("Territory name to filter by (e.g. 'WG AREA', 'SOUTH WEST'). Case-insensitive partial match. If omitted, searches all territories."),
    minEmployees: z.number().optional().default(0).describe("Minimum employee count (Prospect's 'Employees' field, often pupil numbers for schools). Default 0."),
    users: z.array(z.string()).describe("Users to check for tasks — accepts user codes (e.g. 'ML') OR full/partial names (e.g. 'Miles Liesching', 'Miles', 'Al White'). The report finds accounts that have NO open task assigned to ANY of these users."),
    includeClosedTasks: z.boolean().optional().default(false).describe("If true, any task (open or closed) counts. If false (default), only open tasks count — an account with only closed tasks for these users will still appear in the report."),
    maxResults: z.number().optional().default(200).describe("Cap the number of results returned (default 200). Set higher if you need the full list."),
});
...
// ─── Handlers ─────────────────────────────────────────────────
export async function reportAccountsWithoutTasks(args) {
    const client = getClient();
    ...
    if (args.territoryDescription) {
        const territories = await client.get("Territories", `$filter=Obsolete eq 0&$select=TerritoryId,Description`);
        const searchTerm = args.territoryDescription.toUpperCase();
        const match = territories.value.find((t) => t.Description.toUpperCase().includes(searchTerm));
        if (!match) {
    ...
    if (territoryFilter)
        divFilters.push(territoryFilter);
    if (args.minEmployees > 0)
        divFilters.push(`Employees ge ${args.minEmployees}`);
    const divParams = [
        `$filter=${divFilters.join(" and ")}`,
        `$select=DivisionId,Name,Employees,TerritoryCode,PhoneNumber,Website,Relationship`,
        `$orderby=Name`,
    ].join("&");
    const allDivisions = await fetchAll("Divisions", divParams);
    ...
    const allTasks = await fetchAll("Tasks", taskParams);
    // Step 4: Build set of DivisionIds that HAVE tasks
    const divisionsWithTasks = new Set();
    ...
    const results = allDivisions.filter((d) => !divisionsWithTasks.has(d.DivisionId));
...
export async function searchTasks(args) {
    ...
    if (args.openOnly)
        filters.push("ClosedDate eq null");
    if (args.dateFrom)
        filters.push(`TaskDateUtc ge ${args.dateFrom}`);
    if (args.dateTo)
        filters.push(`TaskDateUtc le ${args.dateTo}`);
    ...
    if (args.taskTypeId) {
        ...
        const types = await client.get("TaskTypes", "$select=TaskTypeId,Description&$filter=Obsolete eq false");
    ...
    const lines = result.value.map((t) => {
        const assignee = t.AssignedToUser?.UserName || "N/A";
        const division = t.Division?.Name || "N/A";
        const taskType = t.TaskType?.Description || t.TaskTypeId || "N/A";
        const contact = t.Contact
            ? `${t.Contact.Forename || ""} ${t.Contact.Surname || ""}`.trim()
            : "N/A";
        const date = t.TaskDateUtc?.substring(0, 10) || "N/A";
        const closed = t.ClosedDate ? t.ClosedDate.substring(0, 10) : "Open";
        return [
            ...
            t.Description ? `  Notes: ${t.Description.substring(0, 100)}...` : "",
        ].filter(Boolean).join("\n");
    });
    return `Found ${result.value.length} task(s):\n\n${lines.join("\n\n")}`;
}
export async function getTerritories(args) {
    const client = getClient();
    ...
    const result = await client.get("Territories", params);
    if (result.value.length === 0)
        return "No territories found.";
    const lines = result.value.map((t) => `- \`${t.TerritoryId}\` — ${t.Description}`);
...
export async function createTask(args) {
    ...
    if (args.divisionId !== undefined)
        body.DivisionId = args.divisionId;
...
export async function updateTask(args) {
    ...
    if (fields.taskTypeId !== undefined)
        body.TaskTypeId = fields.taskTypeId;
    if (fields.taskDateUtc !== undefined)
        body.TaskDateUtc = fields.taskDateUtc;
    if (fields.taskTimeZone !== undefined)
        body.TaskTimeZone = fields.taskTimeZone;
    if (fields.description !== undefined)
        body.Description = fields.description;
    if (fields.divisionId !== undefined)
        body.DivisionId = fields.divisionId;
    ...
    if (fields.flagged !== undefined)
        body.Flagged = fields.flagged;
    if (fields.pinned !== undefined)
        body.Pinned = fields.pinned;
    if (fields.closedDate !== undefined)
        body.ClosedDate = fields.closedDate;
...
export async function reportDivisionSummary(args) {
    ...
    if (args.minEmployees > 0)
        filters.push(`Employees ge ${args.minEmployees}`);
    if (args.maxEmployees !== undefined)
        filters.push(`Employees le ${args.maxEmployees}`);
    if (args.relationship)
        filters.push(`contains(Relationship,'${args.relationship}')`);
    if (args.hasWebsite === true)
        filters.push("Website ne null");
    if (args.hasWebsite === false)
        filters.push("Website eq null");
    const params = [
        `$filter=${filters.join(" and ")}`,
    ...
    const result = await client.get("Divisions", params);
    const total = result["@odata.count"];
    if (result.value.length === 0)
        return "No accounts found matching the criteria.";
    const lines = result.value.map((d, i) => {
        const am = d.AccountManagerUser?.UserName || d.AccountManager || "N/A";
        return [
