...
// ─── Handlers ─────────────────────────────────────────────────
export async function searchSystemOptions(args) {
    const client = getClient();
    const filters = ["Visible eq 1"];
    if (args.option)
        filters.push(`contains(Option,'${args.option}')`);
    if (args.group)
        filters.push(`Group eq '${args.group}'`);
    const params = [
    ...
    const result = await client.get("SystemOptions", params);
    if (result.value.length === 0)
        return "No system options found.";
    const lines = result.value.map((o) => {
        const val = o.Value?.substring(0, 80) || "N/A";
        return `- **${o.Option}** = \`${val}\`${o.Value?.length > 80 ? "..." : ""}\n  ${o.Description || "N/A"}${o.Group ? ` [${o.Group}]` : ""}`;
    });
    return `## System Options (${result.value.length})\n\n${lines.join("\n\n")}`;
}
export async function getEntityFields(args) {
    const client = getClient();
    const filters = [`EntityId eq '${args.entityId}'`, "Obsolete eq false"];
    if (args.customOnly)
        filters.push("Core eq false");
    const params = [
        `$filter=${filters.join(" and ")}`,
    ...
    const result = await client.get("EntityFields", params);
    if (result.value.length === 0)
        return `No fields found for entity "${args.entityId}".`;
    ...
    if (custom.length > 0) {
        const customLines = custom.map((f) => `- **${f.FieldName}** (column: ${f.ColumnName})${f.ApiVisible ? "" : " [hidden from API]"}${f.Identifier ? " [identifier]" : ""}`);
        output += `\n## Custom Fields (${custom.length})\n${customLines.join("\n")}`;
    }
    if (!args.customOnly && core.length > 0) {
        const coreLines = core.map((f) => `- **${f.FieldName}** (column: ${f.ColumnName})${f.ApiVisible ? "" : " [hidden]"}`);
        output += `\n\n## Core Fields (${core.length})\n${coreLines.join("\n")}`;
    }
    return output;
}
export async function getEntityLayout(args) {
    const client = getClient();
    const params = [
    ...
    const result = await client.get("EntityLayouts", params);
    if (result.value.length === 0)
        return `No layout found for entity "${args.entityId}".`;
    const lines = result.value.map((l) => `- **${l.EntityId}** v${l.Version} — Dictionary: ${l.Dictionary || "N/A"}`);
