...
export const divisionFiltersSchema = z
    ...
    territoryCode: z.string().optional().describe("AREA LOCATION — case-insensitive exact match on Division.TerritoryCode (e.g. 'WGAREA137e6eff02e14d98942fe6b8baf5af77' or the human label like 'WG AREA' if upstream stores hashed codes)."),
...
export const searchDivisionsSchema = z.object({
    ...
    filters: divisionFiltersSchema.optional().describe("Optional structured filters that AND with searchTerm. Use this to narrow by CustomerType, Relationship, TerritoryCode, AccountManager, or postcode prefix."),
});
export const listDivisionsSchema = z.object({
    filters: divisionFiltersSchema.optional().describe("Structured filters: customerType, relationship, territoryCode, accountManager, postcode (prefix). All AND together."),
    fields: z.array(z.string()).optional().describe("Which Division fields to return. Defaults to the dedupe-relevant subset: " +
    ...
    skip: z.number().int().min(0).optional().describe("If set, returns one page starting at this offset. If omitted, auto-paginates up to 5000 records."),
});
...
export async function searchContacts(args) {
    ...
    const result = await client.get("Contacts", params);
...
export async function getProductDetail(args) {
    ...
    const select = [
        ...
        "SalesAnalysis", "PurchaseAnalysis", "VatCode", "Type", "CatalogueCode",
    ...
    const fenced = (label, raw) => {
        const text = typeof raw === "string" ? raw.trim() : "";
        if (!text)
            return null;
        return `### ${label}\n\`\`\`\n${text}\n\`\`\``;
    };
    const money = (raw) => typeof raw === "number" ? `£${raw.toFixed(2)}` : "N/A";
 ...
 */
function buildDivisionFilterClauses(filters) {
    const clauses = [];
    if (!filters)
        return clauses;
...
const DIVISION_XTRA_EXPAND = "DivisionXtra($select=StandardDropdownField1,StandardDropdownField2,StandardDropdownField3,StandardDropdownField4,StandardDropdownField5)";
/**
 ...
 */
function flattenDropdowns(record) {
    const xtra = record["DivisionXtra"];
    const out = {};
    for (let i = 1; i <= 5; i++) {
        const v = xtra ? xtra[`StandardDropdownField${i}`] : undefined;
 ...
 * are safe.
 */
async function paginateDivisions(baseFilter, selectValue, expandValue, orderbyValue, ceiling, pageSize, startSkip = 0) {
    const client = getClient();
    const records = [];
    ...
    for (let offset = 0; offset < ceiling; offset += effectivePageSize) {
        ...
        const result = await client.get("Divisions", queryStr);
...
export async function searchDivisions(args) {
    const term = args.searchTerm;
    ...
    const { records, totalCount, truncated } = await paginateDivisions(baseFilter, selectValue, expandValue, orderbyValue, ceiling, DIVISION_SERVER_PAGE_CAP);
    if (records.length === 0) {
...
export async function listDivisions(args) {
    ...
    const odataFields = Array.from(new Set(["DivisionId", ...fields.filter((f) => !SYNTHETIC_FIELDS.has(f))]));
    ...
    const expandParts = [];
    if (includePostcode)
        expandParts.push("Address($select=Postcode)");
    if (includeDropdowns)
        expandParts.push(DIVISION_XTRA_EXPAND);
    ...
    if (args.skip !== undefined) {
        const { records: r, totalCount: c, truncated: t } = await paginateDivisions(baseFilter, selectValue, expandValue, orderbyValue, pageSize, pageSize, args.skip);
        records = r;
        totalCount = c;
        truncated = t;
    }
    else {
        const result = await paginateDivisions(baseFilter, selectValue, expandValue, orderbyValue, DIVISION_AUTO_PAGINATE_CEILING, pageSize);
...
export async function getQuoteStatuses() {
    const client = getClient();
    const params = "$select=QuoteStatusCode,Description,DeadFlag&$orderby=QuoteStatusCode";
    const result = await client.get("QuoteStatus", params);
    if (result.value.length === 0) {
