...
export async function createEnquiry(args) {
    ...
    if (args.forename !== undefined)
        body.Forename = args.forename;
    ...
    if (args.companyName !== undefined)
        body.CompanyName = args.companyName;
    if (args.website !== undefined)
        body.Website = args.website;
    if (args.phoneNumber !== undefined)
        body.PhoneNumber = args.phoneNumber;
    if (args.mobileNumber !== undefined)
        body.MobileNumber = args.mobileNumber;
    if (args.description !== undefined)
        body.Description = args.description;
    if (args.source !== undefined)
        body.Source = args.source;
    if (args.enquiryFormId !== undefined)
        body.EnquiryFormId = args.enquiryFormId;
    if (args.sourceTypeCode !== undefined)
        body.SourceTypeCode = args.sourceTypeCode;
    if (args.utmSource !== undefined)
        body.UtmSource = args.utmSource;
    if (args.utmMedium !== undefined)
        body.UtmMedium = args.utmMedium;
    if (args.utmCampaign !== undefined)
        body.UtmCampaign = args.utmCampaign;
    if (args.utmTerm !== undefined)
        body.UtmTerm = args.utmTerm;
    if (args.utmContent !== undefined)
        body.UtmContent = args.utmContent;
    if (args.addressLine1 !== undefined)
        body.AddressLine1 = args.addressLine1;
    if (args.addressLine2 !== undefined)
        body.AddressLine2 = args.addressLine2;
    if (args.addressLine3 !== undefined)
        body.AddressLine3 = args.addressLine3;
...
export async function updateEnquiry(args) {
    ...
    if (fields.mobileNumber !== undefined)
        body.MobileNumber = fields.mobileNumber;
...
export async function searchEnquiries(args) {
    ...
    if (args.forename)
        filters.push(`contains(Forename,'${args.forename}')`);
    if (args.surname)
        filters.push(`contains(Surname,'${args.surname}')`);
    if (args.companyName)
        filters.push(`contains(CompanyName,'${args.companyName}')`);
    if (args.email)
        filters.push(`contains(Email,'${args.email}')`);
    if (args.source)
        filters.push(`contains(Source,'${args.source}')`);
    if (args.dateFrom)
        filters.push(`Created ge ${args.dateFrom}`);
    if (args.dateTo)
        filters.push(`Created le ${args.dateTo}`);
    if (args.convertedOnly)
        filters.push("ConvertedDate ne null");
    if (args.unconvertedOnly)
        filters.push("ConvertedDate eq null");
    ...
    const result = await client.get("Enquiries", params);
    if (result.value.length === 0)
        return "No enquiries found matching the criteria.";
    const lines = result.value.map((e) => {
        const name = `${e.Forename || ""} ${e.Surname || ""}`.trim() || "N/A";
        const assignee = e.AssignedToUser?.UserName || "Unassigned";
        const division = e.Division?.Name || "";
        const converted = e.ConvertedDate ? `Converted ${e.ConvertedDate.substring(0, 10)}` : "Unconverted";
        ...
        return [
            ...
            `  Created: ${e.Created?.substring(0, 10) || "N/A"}${division ? ` | Linked to: ${division}` : ""}`,
            desc ? `  ${desc}` : "",
        ].filter(Boolean).join("\n");
    });
    return `Found ${result.value.length} enquiry(ies):\n\n${lines.join("\n\n")}`;
}
export async function getEnquiry(args) {
    const client = getClient();
    const expand = [
    ...
    const e = await client.getById("Enquiries", args.enquiryId, `$expand=${expand}`);
    const name = `${e.Forename || ""} ${e.Surname || ""}`.trim() || "N/A";
    const assignee = e.AssignedToUser?.UserName || "Unassigned";
    const convertedBy = e.ConvertedByUser?.UserName || "";
    ...
    return [
        ...
        `**Created:** ${e.Created?.substring(0, 10) || "N/A"}`,
        "",
        `## Conversion`,
        e.ConvertedDate
            ? `**Converted:** ${e.ConvertedDate.substring(0, 10)} by ${convertedBy}`
            : "**Status:** Unconverted",
