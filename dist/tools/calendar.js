...
// ─── Handlers ─────────────────────────────────────────────────
export async function searchCalendarEvents(args) {
    const client = getClient();
    const filters = [];
    if (args.subject)
        filters.push(`contains(Subject,'${args.subject}')`);
    if (args.ownerEmail)
        filters.push(`OwnerEmail eq '${args.ownerEmail}'`);
    if (args.dateFrom)
    ...
    const result = await client.get("CalendarEvents", params);
    if (result.value.length === 0)
        return "No calendar events found matching the criteria.";
    const lines = result.value.map((e) => {
        const start = e.StartDate?.substring(0, 16).replace("T", " ") || "N/A";
        const end = e.EndDate?.substring(0, 16).replace("T", " ") || "N/A";
        return [
...
export async function getCalendarEvent(args) {
    const client = getClient();
    const e = await client.getById("CalendarEvents", `'${args.eventId}'`, `$select=EventId,Subject,Description,StartDate,EndDate,StartDateTimeZone,EndDateTimeZone,FreeBusyStatusId,LocationNames,OwnerEmail,CategoryIds,AttendeesRequired,AttendeesOptional,SystemName`);
    return [
        `# Calendar Event`,
        `**Subject:** ${e.Subject || "N/A"}`,
        `**EventId:** ${e.EventId}`,
        `**Start:** ${e.StartDate?.substring(0, 16).replace("T", " ") || "N/A"} (${e.StartDateTimeZone || "N/A"})`,
        `**End:** ${e.EndDate?.substring(0, 16).replace("T", " ") || "N/A"} (${e.EndDateTimeZone || "N/A"})`,
