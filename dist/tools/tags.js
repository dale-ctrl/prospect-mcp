...
export async function getTags(args) {
    ...
    const result = await client.get("Tags", params);
    if (result.value.length === 0)
        return "No tags found.";
    const lines = result.value.map((t) => `- \`${t.TagId}\` — **${t.Description}** (entity: ${t.EntityId || "any"})`);
    return `## Tags (${result.value.length})\n${lines.join("\n")}`;
}
export async function searchTagAssignments(args) {
    const client = getClient();
    const filters = [];
    // Resolve tag description to TagId if needed
    let tagId = args.tagId;
    if (!tagId && args.tagDescription) {
        const tags = await client.get("Tags", "$select=TagId,Description&$filter=Obsolete eq false");
    ...
    const result = await client.get("TagAssignments", params);
    if (result.value.length === 0)
        return "No tag assignments found matching the criteria.";
    const lines = result.value.map((a) => {
        const tagName = a.Tag?.Description || a.TagId;
        const refs = [];
        if (a.ContactId)
            refs.push(`Contact #${a.ContactId}`);
        if (a.DivisionId)
            refs.push(`Division #${a.DivisionId}`);
        if (a.LeadId)
            refs.push(`Lead #${a.LeadId}`);
        if (a.QuoteId)
            refs.push(`Quote #${a.QuoteId}`);
        if (a.CampaignId)
            refs.push(`Campaign #${a.CampaignId}`);
        if (a.ProductItemId)
            refs.push(`Product ${a.ProductItemId}`);
        if (a.ProblemId)
            refs.push(`Problem #${a.ProblemId}`);
        if (a.DocumentId)
            refs.push(`Document #${a.DocumentId}`);
