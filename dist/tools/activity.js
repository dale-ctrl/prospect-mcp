...
// ─── Handlers ─────────────────────────────────────────────────
export async function searchActivityFeed(args) {
    const client = getClient();
    const filters = [];
    if (args.divisionId)
    ...
    const result = await client.get("ActivityFeeds", params);
    if (result.value.length === 0)
        return "No activity feed entries found.";
    const lines = result.value.map((a) => {
        const user = a.ActionUser?.UserName || a.ActionUserCode || "N/A";
        const date = a.Created?.substring(0, 16).replace("T", " ") || "N/A";
        const data = a.Data ? a.Data.substring(0, 120) : "";
        const refs = [];
        if (a.DivisionId)
            refs.push(`Div:${a.DivisionId}`);
        if (a.ContactId)
            refs.push(`Contact:${a.ContactId}`);
        if (a.LeadId)
            refs.push(`Lead:${a.LeadId}`);
        if (a.ProblemId)
            refs.push(`Problem:${a.ProblemId}`);
        if (a.EnquiryId)
            refs.push(`Enquiry:${a.EnquiryId}`);
        return [
...
export async function searchSpokeHistory(args) {
    const client = getClient();
    const filters = [];
    if (args.contactId)
        filters.push(`ContactId eq ${args.contactId}`);
    ...
    if (args.leadId)
        filters.push(`LeadId eq ${args.leadId}`);
    if (args.dateFrom)
        filters.push(`Start ge ${args.dateFrom}`);
    if (args.dateTo)
        filters.push(`Start le ${args.dateTo}`);
    if (args.user) {
        const code = await resolveUser(args.user);
    ...
    const result = await client.get("SpokeHistories", params);
    if (result.value.length === 0)
        return "No spoke history found.";
    const lines = result.value.map((s) => {
        const start = s.Start?.substring(0, 16).replace("T", " ") || "N/A";
        const finish = s.Finish?.substring(0, 16).replace("T", " ") || "";
        const transferred = s.TransferredTo ? ` → ${s.TransferredTo}` : "";
        const refs = [];
        if (s.DivisionId)
            refs.push(`Div:${s.DivisionId}`);
        if (s.ContactId)
            refs.push(`Contact:${s.ContactId}`);
        if (s.LeadId)
            refs.push(`Lead:${s.LeadId}`);
...
export async function searchRecalls(args) {
    const client = getClient();
    ...
    if (args.dateFrom)
        filters.push(`RecallDate ge ${args.dateFrom}`);
    if (args.dateTo)
        filters.push(`RecallDate le ${args.dateTo}`);
    if (args.overdueOnly)
        filters.push(`RecallDate lt ${new Date().toISOString().substring(0, 10)}`);
    if (args.user) {
        const code = await resolveUser(args.user);
    ...
    if (result.value.length === 0)
        return `No ${args.entity} recalls found.`;
    const lines = result.value.map((r) => {
        const user = r.RecallUser?.UserName || r.RecallUserId || "N/A";
        const date = r.RecallDate?.substring(0, 10) || "N/A";
        const isOverdue = new Date(r.RecallDate) < new Date();
