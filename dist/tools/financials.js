...
// ─── Handlers ─────────────────────────────────────────────────
export async function searchSalesInvoices(args) {
    const client = getClient();
    // OperatingCompanyCode is part of the composite key — required to scope the collection.
    const filters = [`OperatingCompanyCode eq 'A'`];
    if (args.invoiceNumber)
        filters.push(`contains(InvoiceNumber,'${args.invoiceNumber}')`);
    if (args.salesLedgerId)
        filters.push(`SalesLedgerId eq '${args.salesLedgerId}'`);
    if (args.divisionId) {
        // SalesInvoiceHeader has no DivisionId field — resolve to SalesLedgerId via Division lookup.
        const div = await client.getById("Divisions", args.divisionId, "$select=SalesLedgerId");
        const ledgerId = div.SalesLedgerId;
        if (ledgerId)
            filters.push(`SalesLedgerId eq '${ledgerId}'`);
    }
    if (args.dateFrom)
        filters.push(`InvoiceDate ge ${args.dateFrom}`);
    if (args.dateTo)
        filters.push(`InvoiceDate le ${args.dateTo}`);
    if (args.minValue)
    ...
    const result = await client.get("SalesInvoiceHeaders", params);
    if (result.value.length === 0)
        return "No sales invoices found matching the criteria.";
    const lines = result.value.map((inv) => {
        const date = inv.InvoiceDate?.substring(0, 10) || "N/A";
        const due = inv.DueDate?.substring(0, 10) || "N/A";
        return [
            `**Invoice ${inv.InvoiceNumber}**${inv.CreditNoteNumber ? ` (Credit: ${inv.CreditNoteNumber})` : ""}`,
            `  Account: ${inv.SalesLedgerId || "N/A"}`,
            `  Date: ${date} | Due: ${due}`,
            `  Net: £${inv.BaseNetValue?.toFixed(2) ?? "0.00"} | Tax: £${inv.BaseTaxValue?.toFixed(2) ?? "0.00"} | Gross: £${inv.BaseGrossValue?.toFixed(2) ?? "0.00"}`,
            `  Customer Ref: ${inv.CustomerReference || "N/A"}`,
        ].join("\n");
    });
    return `Found ${result.value.length} invoice(s):\n\n${lines.join("\n\n")}`;
}
export async function getSalesInvoice(args) {
    const client = getClient();
    const opCode = args.operatingCompanyCode || "A";
    // Composite key: OperatingCompanyCode + InvoiceNumber
    const inv = await client.getById("SalesInvoiceHeaders", `OperatingCompanyCode='${opCode}',InvoiceNumber='${args.invoiceNumber}'`);
    return [
        ...
        "",
        `## Dates`,
        `- Invoice Date: ${inv.InvoiceDate?.substring(0, 10) || "N/A"}`,
        `- Due Date: ${inv.DueDate?.substring(0, 10) || "N/A"}`,
        "",
        `## Values`,
        `- Net: £${inv.NetValue?.toFixed(2) ?? "0.00"}`,
        `- Tax: £${inv.TaxValue?.toFixed(2) ?? "0.00"}`,
        `- Gross: £${inv.GrossValue?.toFixed(2) ?? "0.00"}`,
        "",
        `## Base Currency Values`,
        `- Base Net: £${inv.BaseNetValue?.toFixed(2) ?? "0.00"}`,
        `- Base Tax: £${inv.BaseTaxValue?.toFixed(2) ?? "0.00"}`,
        `- Base Gross: £${inv.BaseGrossValue?.toFixed(2) ?? "0.00"}`,
        "",
        `**Order ID:** ${inv.SalesOrderHeaderId || "N/A"}`,
        `**Accounts ID:** ${inv.AccountsId || "N/A"}`,
    ].filter(Boolean).join("\n");
}
export async function searchSalesTransactions(args) {
    const client = getClient();
    const filters = [];
    if (args.invoiceNumber)
        filters.push(`contains(InvoiceNumber,'${args.invoiceNumber}')`);
    ...
    if (args.productItemId)
        filters.push(`contains(ProductItemId,'${args.productItemId}')`);
    if (args.productDescription)
        filters.push(`contains(ProductDescription,'${args.productDescription}')`);
    if (args.divisionId) {
        // SalesTransaction has no DivisionId field — resolve to account code via Division lookup.
        const div = await client.getById("Divisions", args.divisionId, "$select=SalesLedgerId");
        const ledgerId = div.SalesLedgerId;
        if (ledgerId)
            filters.push(`Account eq '${ledgerId}'`);
    }
    if (args.salesLedgerId)
        filters.push(`Account eq '${args.salesLedgerId}'`);
    if (args.dateFrom)
        filters.push(`InvoiceDate ge ${args.dateFrom}`);
    if (args.dateTo)
        filters.push(`InvoiceDate le ${args.dateTo}`);
    ...
    const result = await client.get("SalesTransactions", params);
    if (result.value.length === 0)
        return "No sales transactions found matching the criteria.";
    const lines = result.value.map((t) => {
        const date = t.InvoiceDate?.substring(0, 10) || "N/A";
        return [
            ...
            `  Type: ${t.TransactionType || "N/A"} | Qty: ${t.CalculatedInvoiceQuantity ?? 0} x £${t.CalculatedPrice?.toFixed(2) ?? "0.00"} = £${t.CalculatedLineValue?.toFixed(2) ?? "0.00"}`,
            `  Date: ${date}`,
        ].join("\n");
    });
    return `Found ${result.value.length} transaction(s):\n\n${lines.join("\n\n")}`;
}
export async function reportAccountFinancials(args) {
    const client = getClient();
    ...
    const dateFilters = [];
    if (args.dateFrom)
        dateFilters.push(`InvoiceDate ge ${args.dateFrom}`);
    if (args.dateTo)
        dateFilters.push(`InvoiceDate le ${args.dateTo}`);
    // Fetch invoices — SalesInvoiceHeader has no DivisionId; filter by SalesLedgerId + OperatingCompanyCode
    const invFilters = [`OperatingCompanyCode eq 'A'`];
    if (salesLedgerId)
        invFilters.push(`SalesLedgerId eq '${salesLedgerId}'`);
    ...
    const invoices = await client.get("SalesInvoiceHeaders", invParams);
    // Fetch transactions — SalesTransaction has no DivisionId; filter by Account (= SalesLedgerId)
    const txFilters = [];
    if (salesLedgerId)
        txFilters.push(`Account eq '${salesLedgerId}'`);
    ...
    const transactions = await client.get("SalesTransactions", txParams);
    // Calculate totals
    let totalNet = 0;
    let totalGross = 0;
    for (const inv of invoices.value) {
        totalNet += inv.NetValue || 0;
        totalGross += inv.GrossValue || 0;
    }
    let output = [
    ...
    if (invoices.value.length > 0) {
        ...
        const invLines = invoices.value.slice(0, 20).map((inv) => {
            const date = inv.InvoiceDate?.substring(0, 10) || "N/A";
            return `- ${inv.InvoiceNumber}${inv.CreditNoteNumber ? ` (CN: ${inv.CreditNoteNumber})` : ""} | ${date} | Net: £${inv.NetValue?.toFixed(2) ?? "0.00"} | Ref: ${inv.CustomerReference || "N/A"}`;
        });
    ...
    if (transactions.value.length > 0) {
        ...
        const txLines = transactions.value.slice(0, 30).map((t) => {
            const date = t.InvoiceDate?.substring(0, 10) || "N/A";
            return `- ${t.ProductItemId || "—"}: ${t.ProductDescription || "N/A"} | ${t.TransactionType || "N/A"} | Qty: ${t.CalculatedInvoiceQuantity ?? 0} x £${t.CalculatedPrice?.toFixed(2) ?? "0.00"} = £${t.CalculatedLineValue?.toFixed(2) ?? "0.00"} | Inv: ${t.InvoiceNumber || "N/A"} | ${date}`;
        });
        output += txLines.join("\n");
    }
    return output;
