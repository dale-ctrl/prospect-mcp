...
export async function createProduct(args) {
    ...
    const check = await client.get("ProductItems", `$filter=ProductItemId eq '${code}'&$select=ProductItemId,Description,DecimalSellingPrice,DecimalCostPrice,Manufacturer,ManufacturerReference,CategoryId,VatCode,PurchaseAnalysis,SalesAnalysis,Type,Obsolete,CatalogueCode`);
...
export const updateProductXtraSchema = z.object({
    productItemId: z
        .string()
        .describe("Product code / SKU (ProductItemId) whose ProductItemXtra row to update (matches the ProductItem this Xtra row hangs off of, e.g. 'NC17062601')."),
    fields: z
...
async function upsertProductItemXtra(productItemId, body) {
    ...
    const keyExpr = `OperatingCompanyCode='${OPERATING_COMPANY_CODE}',ProductItemId='${escapedCode}'`;
    try {
        await client.patch(PRODUCT_ITEM_XTRAS_ENTITY_SET, keyExpr, body);
    }
    catch (err) {
    ...
    sp.set("$top", "1");
    const result = await client.get(PRODUCT_ITEM_XTRAS_ENTITY_SET, sp.toString());
...
export async function updateProductXtra(input) {
