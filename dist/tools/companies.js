...
export const updateCompanySchema = z.object({
    ...
    companyGroupType: z.string().optional().describe("Patches Company.TypeId (Group Type). Accepts the FK code (e.g. 'CUS') or the UI label ('Customer')."),
...
export const listCompaniesSchema = z.object({
    filters: z
        ...
        .optional(),
    fields: z.array(z.string()).optional().describe("Which Company fields to return. Defaults to: CompanyId, Name, TypeId, AccountManagerId, Source, AlternateReference, LastUpdated."),
...
export async function getCompany(args) {
    ...
    const result = await client.getById("Companies", args.companyId, buildQuery({ $expand: "Type($select=Code,Description)" }));
...
export async function updateCompany(args) {
    ...
    await client.patch("Companies", companyId, body);
...
export async function listCompanies(args) {
    ...
    const filterClauses = ["StatusFlag ne 'D'"];
    ...
    for (let offset = 0; offset < ceiling; offset += effectivePageSize) {
        ...
        const result = await client.get("Companies", queryStr);
