...
export const deleteContactSchema = z.object({
    contactId: z.number().int().positive().describe("ContactId to soft-delete. Refuses with a listing if the contact has live Quotes / Leads / Tasks (clean those up first or use the UI)."),
});
export const mergeDivisionSchema = z.object({
    ...
    deleteSource: z.boolean().optional().default(false).describe("When true, soft-delete the source Division after the merge (StatusFlag='D'). Defaults to false so the caller can verify before deletion."),
});
 ...
 */
async function patchSafe(entitySet, id, body) {
    try {
        const client = getClient();
        await client.patch(entitySet, id, body);
        return { ok: true };
    }
    catch (err) {
...
export async function deleteTask(args) {
    const client = getClient();
    const before = await client.get("Tasks", `$filter=TaskId eq ${args.taskId}&$select=TaskId,Name,StatusFlag&$top=1`);
...
export async function deleteEnquiry(args) {
    const client = getClient();
    const enquiry = await client.getById("Enquiries", args.enquiryId, "$select=EnquiryId,Forename,Surname,StatusFlag,ConvertedDate,LeadId");
    if (enquiry.StatusFlag === "D") {
        return `Enquiry ${args.enquiryId} is already deleted. No change.`;
    }
    if (enquiry.ConvertedDate || enquiry.LeadId) {
        throw new Error(`Enquiry ${args.enquiryId} (${enquiry.Forename ?? ""} ${enquiry.Surname ?? ""}`.trim() +
...
export async function deleteActivityNote(args) {
    const client = getClient();
    const before = await client.get("Notepads", `$filter=NotepadId eq ${args.noteId}&$select=NotepadId,StatusFlag&$top=1`);
    if (before.value.length === 0) {
...
export async function deleteContact(args) {
    const client = getClient();
    const contact = await client.getById("Contacts", args.contactId, "$select=ContactId,Forename,Surname,StatusFlag");
    ...
    const [quotes, leads, tasks] = await Promise.all([
        client.get("Quotes", `$filter=${filterActive}&$select=QuoteId,Description&$top=20&$count=true`),
        client.get("Leads", `$filter=${filterActive}&$select=LeadId,Description&$top=20&$count=true`),
        client.get("Tasks", `$filter=${filterActive}&$select=TaskId,Name&$top=20&$count=true`),
    ]);
    const qCount = quotes["@odata.count"] ?? quotes.value.length;
    const lCount = leads["@odata.count"] ?? leads.value.length;
    const tCount = tasks["@odata.count"] ?? tasks.value.length;
    ...
    if (total > 0) {
        ...
        if (qCount > 0) {
            ...
            for (const q of quotes.value.slice(0, 5))
                lines.push(`  - ${q.QuoteId}: ${q.Description ?? "(no description)"}`);
            if (qCount > 5)
                lines.push(`  … and ${qCount - 5} more`);
        }
        if (lCount > 0) {
            lines.push(`**${lCount} Lead(s):**`);
            for (const l of leads.value.slice(0, 5))
                lines.push(`  - ${l.LeadId}: ${l.Description ?? "(no description)"}`);
            if (lCount > 5)
                lines.push(`  … and ${lCount - 5} more`);
        }
        if (tCount > 0) {
            lines.push(`**${tCount} Task(s):**`);
            for (const t of tasks.value.slice(0, 5))
                lines.push(`  - ${t.TaskId}: ${t.Name ?? "(no name)"}`);
            if (tCount > 5)
                lines.push(`  … and ${tCount - 5} more`);
        }
...
// ─── Merge / move handlers ────────────────────────────────────
async function moveChildren(entitySet, filter, selectKey, patchBody) {
    const client = getClient();
    const result = await client.get(entitySet, `$filter=${filter}&$select=${selectKey}&$top=500`);
    let moved = 0;
    let failed = 0;
    const failures = [];
    for (const row of result.value) {
        const id = row[selectKey];
...
export async function mergeDivision(args) {
    const client = getClient();
    ...
    const [src, tgt] = await Promise.all([
        client.getById("Divisions", args.sourceDivisionId, "$select=DivisionId,Name,StatusFlag"),
        client.getById("Divisions", args.targetDivisionId, "$select=DivisionId,Name,StatusFlag"),
    ]);
    if (src.StatusFlag === "D")
        throw new Error(`Source Division ${src.DivisionId} is already deleted.`);
    if (tgt.StatusFlag === "D")
        throw new Error(`Target Division ${tgt.DivisionId} is deleted — cannot merge into it.`);
    const filterActive = (col) => `${col} eq ${args.sourceDivisionId} and StatusFlag ne 'D'`;
    const contacts = await moveChildren("Contacts", filterActive("DivisionId"), "ContactId", { DivisionId: args.targetDivisionId });
    const tasks = await moveChildren("Tasks", filterActive("DivisionId"), "TaskId", { DivisionId: args.targetDivisionId });
    const enquiries = await moveChildren("Enquiries", filterActive("DivisionId"), "EnquiryId", { DivisionId: args.targetDivisionId });
    const leads = await moveChildren("Leads", filterActive("DivisionId"), "LeadId", { DivisionId: args.targetDivisionId });
    const quotes = await moveChildren("Quotes", filterActive("DivisionId"), "QuoteId", { DivisionId: args.targetDivisionId });
    ...
    const notes = await moveChildren("Notepads", `DivisionId eq ${args.sourceDivisionId} and ObjectType eq 'division' and StatusFlag ne 'D'`, "NotepadId", { DivisionId: args.targetDivisionId, ObjectId: String(args.targetDivisionId) });
    ...
    if (args.deleteSource) {
        ...
        const totalFailed = contacts.failed + tasks.failed + enquiries.failed + leads.failed + quotes.failed + notes.failed;
        if (totalFailed === 0) {
...
export async function reparentDivision(args) {
    const client = getClient();
    const division = await client.getById("Divisions", args.divisionId, "$select=DivisionId,Name,CompanyId,StatusFlag");
    ...
    await client.patch("Divisions", args.divisionId, {
...
export async function moveContact(args) {
    const client = getClient();
    const contact = await client.getById("Contacts", args.contactId, "$select=ContactId,DivisionId,Forename,Surname,StatusFlag");
    if (contact.StatusFlag === "D") {
    ...
    const tgt = await client.getById("Divisions", args.targetDivisionId, "$select=DivisionId,Name,StatusFlag");
    if (tgt.StatusFlag === "D") {
        throw new Error(`Target Division ${args.targetDivisionId} is deleted.`);
    }
    const fromDiv = contact.DivisionId;
    // Move the contact itself.
    await client.patch("Contacts", args.contactId, {
    ...
    // grouping.)
    const tasks = await moveChildren("Tasks", `ContactId eq ${args.contactId} and DivisionId eq ${fromDiv} and StatusFlag ne 'D'`, "TaskId", { DivisionId: args.targetDivisionId });
    const notes = await moveChildren("Notepads", `ContactId eq ${args.contactId} and DivisionId eq ${fromDiv} and StatusFlag ne 'D'`, "NotepadId", { DivisionId: args.targetDivisionId });
    return [
