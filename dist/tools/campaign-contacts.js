...
async function findExistingMembership(campaignActivityId, contactId) {
    const client = getClient();
    const r = await client.get("CampaignActivityContacts", `$filter=CampaignActivityId eq ${campaignActivityId} and ContactId eq ${contactId}&$top=1`);
    return r.value[0];
}
// ─── Schemas ──────────────────────────────────────────────────
export const addContactToCampaignSchema = z.object({
    ...
    campaignActivityId: z.number().int().positive().optional().describe("Specific CampaignActivityId. Optional — defaults to the campaign's lowest-id activity."),
    comments: z.string().optional().describe("Optional Comments field on the join row (free text, up to 32,767 chars). Useful for tagging the source of the import (e.g. 'S&A Show 2026 lead-load')."),
});
...
export const listCampaignContactsSchema = z.object({
    ...
    campaignActivityId: z.number().int().positive().optional().describe("Specific CampaignActivityId. Optional — defaults to the campaign's lowest-id activity."),
    top: z.number().int().positive().optional().default(200).describe("Max rows to return (default 200). Prospect's API caps a single page at 500."),
});
// ─── Handlers ─────────────────────────────────────────────────
export async function addContactToCampaign(args) {
    const client = getClient();
    // Validate contact exists.
    await client.getById("Contacts", args.contactId, "$select=ContactId");
    ...
    await client.post("CampaignActivityContacts", body);
...
export async function removeContactFromCampaign(args) {
    const client = getClient();
...
export async function listCampaignContacts(args) {
    const client = getClient();
    const activity = await resolveCampaignActivity(args.campaignId, args.campaignActivityId);
    const params = [
    ...
    const lines = rows.map((r) => {
        ...
        const division = contact?.Division?.Name || "—";
        const job = contact?.JobTitle ? ` [${contact.JobTitle}]` : "";
        const comment = r.Comments ? ` — "${r.Comments.replace(/\s+/g, " ").slice(0, 80)}"` : "";
        const added = r.Created?.substring(0, 10) || "?";
...
//# sourceMappingURL=campaign-contacts.js.map