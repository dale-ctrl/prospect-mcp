...
export async function searchDocuments(args) {
    ...
    if (args.divisionId)
        filters.push(`Contact/DivisionId eq ${args.divisionId}`);
    if (args.contactId)
    ...
    if (args.emailSubject)
        filters.push(`contains(EmailSubject,'${args.emailSubject}')`);
    if (args.dateFrom)
    ...
    const lines = result.value.map((d) => {
        ...
        const company = contact?.Division?.Name || "";
        const docType = d.DocumentType?.Description || "Unknown";
        const direction = d.Direction === 1 ? "Inbound" : d.Direction === 2 ? "Outbound" : "";
        const date = d.Created?.substring(0, 10) || "N/A";
...
export async function getDocument(args) {
    ...
    const d = await client.getById("Documents", args.documentId, `$expand=${expand}`);
    const contact = d.Contact;
    const contactName = contact ? `${contact.Forename || ""} ${contact.Surname || ""}`.trim() : "N/A";
    const company = contact?.Division?.Name || "N/A";
    const docType = d.DocumentType?.Description || "Unknown";
    const createdBy = d.CreatedByUser?.UserName || "N/A";
    ...
    return [
        ...
        "",
        `**Created:** ${d.Created?.substring(0, 10) || "N/A"} by ${createdBy}`,
        `**Document Date:** ${d.DocumentDate?.substring(0, 10) || "N/A"}`,
    ].filter(Boolean).join("\n");
}
export async function getDocumentTypes() {
    const client = getClient();
    const result = await client.get("DocumentTemplates", "$select=DocumentTypeCode,Description&$orderby=Description&$top=100");
    if (result.value.length === 0)
        return "No document types found.";
    const lines = result.value.map((t) => `- \`${t.DocumentTypeCode}\` — ${t.Description || "(no description)"}`);
