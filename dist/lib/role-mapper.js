...
function matchAgainst(haystack) {
    const lc = haystack.toLowerCase();
    // Rule 1 first.
    const r1 = RULES[0];
    for (const p of r1.patterns) {
        if (lc.includes(p))
            return { rule: r1, matched: p };
    }
 ...
 */
export function resolveContactRole(input) {
 ...
 */
export function resolveRoleCodeOrLabel(input, liveRoles) {
    ...
    const byDesc = liveRoles.find((r) => (r.Description || "").toLowerCase().includes(lc));
    if (byDesc) {
