...
const HARDCODED_SLOTS = SLOT_FAMILIES.flatMap((f) => Array.from({ length: f.count }, (_, i) => ({
...
export function entityIdForXtraSet(entitySetOrId) {
 ...
 */
function columnToIdentifier(column) {
 ...
 */
async function loadSlotLabels(client, entityId) {
    try {
        ...
        sp.set("$filter", `startswith(RowIdentity,'Entity.${entityId}.Standard') and Locale eq '${locale}'`);
        sp.set("$select", "RowIdentity,Value");
        sp.set("$top", "200");
        const result = await client.get("Translations", sp.toString());
        const out = {};
        const prefix = `Entity.${entityId}.`;
        const suffix = `:${locale}`;
        for (const r of result.value) {
            if (!r.Value || !r.RowIdentity)
                continue;
            if (!r.RowIdentity.startsWith(prefix) || !r.RowIdentity.endsWith(suffix))
                continue;
            const fieldName = r.RowIdentity.substring(prefix.length, r.RowIdentity.length - suffix.length);
            if (SLOT_IDENTIFIER_RE.test(fieldName))
 ...
 */
function identifierToColumn(identifier) {
    const m = /^Standard([A-Za-z]+)Field(\d+)$/.exec(identifier);
    if (!m)
        return null;
    return `x_365_custom_${m[1].toLowerCase()}_${m[2]}`;
}
export async function loadXtraSlots(client, entitySetOrId) {
    ...
    const promise = (async () => {
        ...
        const [slotRows, labels] = await Promise.all([
            (async () => {
                ...
                sp.set("$top", "200");
                try {
                    const result = await client.get("EntityFields", sp.toString());
                    return result.value;
                }
                catch {
                    return [];
        ...
        for (const r of slotRows) {
            if (!r.FieldName || !r.ColumnName)
                continue;
            if (!SLOT_IDENTIFIER_RE.test(r.FieldName))
                continue;
...
export function indexSlotsByIdentifier(slots) {
    const out = {};
 ...
 */
export function resolveXtraFieldsToBody(slots, fields) {
    const byIdentifier = new Map();
    const byColumnLower = new Map();
    const byLabelLower = new Map();
    for (const s of slots) {
        byIdentifier.set(s.identifier, s);
        if (s.columnName)
            byColumnLower.set(s.columnName.toLowerCase(), s);
        if (s.fieldLabel)
            byLabelLower.set(s.fieldLabel.toLowerCase(), s);
    }
    const body = {};
    const unknown = [];
    ...
    if (unknown.length > 0) {
        ...
        throw new Error(`Unknown Xtra field(s): ${unknown.map((u) => `"${u}"`).join(", ")}. ` +
 ...
 */
export function columnNameForIdentifier(slots, identifier) {
    for (const s of slots) {
...
export function __resetXtraLabelCache() {
    cache.clear();
}
//# sourceMappingURL=xtra-labels.js.map