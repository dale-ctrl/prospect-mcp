...
const { sendQuoteEmail, getMergeOutput, sendEntityEmail, getDocumentContent } = await import("./tools/quote-messaging.js");
function installFetchMock(handler) {
    ...
    globalThis.fetch = (async (url, init) => {
        ...
        const call = { url: u, method: init?.method ?? "GET", body: rawBody, parsedBody };
        calls.push(call);
        const resp = handler(call);
        const status = resp.status ?? 200;
        const contentType = resp.contentType ?? "application/json";
        const bodyInit = resp.bytes !== undefined
            ? resp.bytes
...
test("sendEntityEmail: runs the 7-call flow with correct bodies and required headers", async () => {
    ...
    const mock = installFetchMock((call) => {
        ...
        if (call.url.endsWith("/Quotes(15234)/SendMessage()")) {
            ...
            assert.match(b.MessageBody, /Rendered body/);
            assert.match(b.MessageBody, /—DL/);
...
test("ProspectClient: sends x-locale and x-profile-id headers when PROSPECT_PROFILE_ID is set", async () => {
    ...
    try {
        ...
        if (origProfile === undefined)
            ...
            process.env.PROSPECT_PROFILE_ID = origProfile;
        if (origLocale === undefined)
            delete process.env.PROSPECT_LOCALE;
        else
            process.env.PROSPECT_LOCALE = origLocale;
    }
});
test("sendEntityEmail: skips steps 3-5 when attachment is omitted (body-only email)", async () => {
    ...
    const mock = installFetchMock((call) => {
        ...
        if (call.url.endsWith("/MergeData()")) {
            mergeCalls++;
            if (mergeCalls === 1)
                return mergeResp({ Subject: "Subj 1", Body: "B" });
            if (mergeCalls === 2)
                return mergeResp({ Signature: "Sig" });
...
test("sendEntityEmail: throws descriptive error when SendMessage returns value:0", async () => {
    ...
    try {
        await assert.rejects(() => sendEntityEmail({
            entitySet: "Quotes",
            entityId: 1,
            to: "x@y.z",
            emailTemplateCode: "_EMLQC",
...
test("sendQuoteEmail: SAFETY GATE — recipient is locked to the API user even when caller omits to/cc/bcc", async () => {
    ...
    const mock = installFetchMock((call) => {
        ...
        if (call.url.endsWith("/MergeData()")) {
            merges++;
            if (merges === 1)
                return mergeResp({ Subject: "Your Quotation Ref: 15234 / 3", Body: "<p>body</p>" });
            if (merges === 2)
                return mergeResp({ Signature: "<br>—DL" });
            if (merges === 3)
                return mergeResp({ DocumentName: "Quote Document 15234" });
...
test("sendQuoteEmail: SAFETY GATE — caller-supplied to/cc/bcc are silently overridden to the API user", async () => {
    ...
    const mock = installFetchMock((call) => {
        ...
        if (call.url.endsWith("/MergeData()")) {
            merges++;
            if (merges === 1)
                return mergeResp({ Subject: "Subj 15234", Body: "B" });
            if (merges === 2)
                return mergeResp({ Signature: "Sig" });
            throw new Error("no merge 3 expected (attachPdf=false)");
        }
        if (call.url.endsWith("/Quotes(15234)/SendMessage()")) {
            sendMessageBody = call.parsedBody;
    ...
    try {
        ...
        assert.equal(sendMessageBody.ToAddress, "apiuser@westcountrygroup.com", "ToAddress leaked caller's foo@example.com — safety gate failed");
        assert.equal(sendMessageBody.CcAddress, "");
        assert.equal(sendMessageBody.BccAddress, "");
...
//# sourceMappingURL=test-messaging.js.map