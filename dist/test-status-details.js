...
const { getLeadLookups, searchOpportunities, getOpportunity, getLeadLookupsSchema, searchOpportunitiesSchema } = await import("./tools/opportunities.js");
...
test("search_opportunities(statusDetail=X) builds combined label/code OData filter", async () => {
    ...
    try {
        ...
        assert.ok(filter.includes("StatusDetail/Description") && filter.includes("StatusDetailId"), `statusDetail filter must include both label-contains and code-eq clauses: ${filter}`);
...
//# sourceMappingURL=test-status-details.js.map