...
const { buildProductSearchFilter, PRODUCT_SEARCH_FIELDS, searchProducts } = await import("./tools/lookups.js");
...
test("search_products: lights up Mfr Ref and Manufacturer in row output when present", async () => {
    ...
    try {
        ...
        for (const field of PRODUCT_SEARCH_FIELDS) {
            assert.ok(filter.includes(`contains(tolower(${field}),'dna140')`), `filter must include ${field}: ${filter}`);
        }
...
//# sourceMappingURL=test-search-products.js.map