...
if (!quoteArg)
    usage("Missing QuoteId argument.");
const quoteId = Number.parseInt(quoteArg, 10);
if (!Number.isFinite(quoteId) || quoteId <= 0)
    usage(`Invalid QuoteId: ${quoteArg}`);
async function main() {
    const client = getClient();
    console.log(`━━━ Step 1: Read quote ${quoteId} current status ━━━`);
    const quote = await client.getById("Quotes", quoteId, "$expand=Status($select=Description)&$select=QuoteId,StatusId,Description,ContactId");
...
//# sourceMappingURL=test-send-message.js.map